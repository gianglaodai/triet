...
> Một ngôn ngữ **tam phân cân bằng** (balanced-ternary-first), thiết kế để không
> bao giờ đóng cánh cửa xuống tới hệ điều hành.
...
> của nó nằm ở **craft** — một tác phẩm kỹ thuật mạch lạc, kỷ luật — KHÔNG ở
> adoption, KHÔNG ở một giả thuyết AI, KHÔNG ở việc chờ một sự kiện phần cứng.
> Tài liệu này cố tình KHÔNG hứa những thứ một dự án một-người không thể giao. Mỗi tham vọng ở đây được dán nhãn rõ là **ràng buộc**
...
Triết là một **tác phẩm craft có chủ đích**: lấy tam phân cân bằng và logic
Łukasiewicz Ł3 làm bản sắc, rồi xây quanh nó một compiler mạch lạc, kỷ luật,
không bao giờ panic. Nó KHÔNG phải một thí nghiệm để chứng minh giả thuyết nào.
...
Giá trị của Triết, nói thẳng, là **một tác phẩm kỹ thuật mạch lạc với kỷ luật
cao** (ADR-driven, schema-first, compiler-không-bao-giờ-panic), neo vào **tính
nhất quán nội tại** của một đại số Ł3 duy nhất chạy xuyên null / logic / capability
(§8). Không phải "thế giới cần một ngôn ngữ thứ N", và **không tuyên bố giả thuyết
AI nào** (xem §5).
...
- **Không tuyên bố là "AI-first".** Triết KHÔNG chứng minh, KHÔNG đo, KHÔNG bán
  bất kỳ giả thuyết AI nào (§5). Nhãn "AI-first" cũ là một sai lầm đặt tên, đã gỡ
  (2026-06-22).
...
## 4. Ba trụ cột thật (phục vụ tính đúng đắn + craft)

Đây là những trụ cột **kiểm chứng được**, biện minh bằng tính đúng đắn của
compiler và sự mạch lạc của tác phẩm — KHÔNG bằng một giả thuyết AI:
...
cấm**. Ít trạng thái ẩn để đoán nhầm — cho người đọc, cho compiler, và (tình cờ) cho cả AI.

### 4.2 — Diagnostic máy-sửa-được (ADR-0027 là hiến pháp)
Mọi lỗi theo format máy-sửa-được khóa ở [ADR-0027](docs/decisions/0027-diagnostic-format-standard.md):
header `EXXXX Tên` + body + span + khối `[Fix N]` mệnh lệnh (`Change/Wrap/Use/Add/
Replace/Move X to Y`). 

Diagnostic được chia làm hai lớp rõ rệt theo việc compiler có biết chắc cách sửa hay không:
...
- **Intent-Dependent**: Lỗi đòi hỏi hiểu ý định của lập trình viên (ví dụ: mismatch kiểu dữ liệu phức tạp, vi phạm borrowck liên quan đến logic). Đây là nơi diagnostic chất lượng cao trả công nhiều nhất cho bất kỳ người sửa nào — con người hay LLM (lợi ích cho LLM là *hệ quả phụ không tuyên bố, không đo* — §5).
...
(conservative). Đây là nguyên tắc soundness, đứng độc lập với mọi giả thuyết AI.

## 5. Nhãn "AI-first" cũ — ĐÃ GỠ (bia mộ trung thực, 2026-06-22)

> **Trạng thái: ĐÃ GỠ. Triết không tuyên bố, không đo, không bán giả thuyết AI nào.**

Một bản trước của tài liệu này dựng "AI-first" thành trụ cột ngang hàng: giả thuyết
rằng ngôn ngữ + diagnostic giúp một LLM **hội tụ về code đúng trong ít lượt hơn**.
Ngày 2026-06-22, tác giả gỡ nhãn này. Hai lý do, cả hai đứng vững:

1. **Nó orthogonal với bản sắc.** Vòng hội tụ (nếu có) đến từ explicit syntax +
   diagnostic máy-sửa-được + refuse-over-guess (§4) — không cái nào *cần* tam phân.
   Phần độc nhất của Triết (tam phân/Ł3) không gánh giả thuyết; phần gánh giả thuyết
   (chất lượng diagnostic) không độc nhất (Rust/Elm/TS đã làm nhiều năm). Hai thứ
   không gặp nhau.
2. **Một bonus không đo chỉ là quảng cáo.** Refuse-over-guess áp cho cả lời tuyên
   bố: chưa đo thì không bán. Tác giả chọn liêm chính của một nghệ sĩ — không gắn
   nhãn "tương lai sẽ đo" để làm đẹp dự án.

**Hệ quả:** các nguyên tắc §4 (explicit / diagnostic / refuse) **ở lại** — nhưng
được biện minh bằng **tính đúng đắn và craft**, không bằng AI. Nếu một ngày chúng
tình cờ giúp một LLM, đó là *hệ quả phụ không tuyên bố, không đo*. Phòng tuyến giá
trị của Triết là **coherence** (§8: một đại số Ł3 duy nhất xuyên null / logic /
capability), KHÔNG phải một con số turns-to-green.
...
Thước đo thành công KHÔNG phải adoption, KHÔNG phải viết được OS, KHÔNG phải một
giả thuyết AI — mà là **một ngôn ngữ + compiler mạch lạc, đúng đắn, kỷ luật, với
một đại số Ł3 nhất quán xuyên null / logic / capability (§8)**. Mọi thứ khác — tam
phân hardware, OS thật, ecosystem, lợi ích cho AI codegen — là *cảm hứng / hệ quả
phụ*, không phải cam kết.
