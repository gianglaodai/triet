...
- **Compile-stage enforcement** (`triet-typecheck::check_capabilities`): cross-root imports (`sys.*`/`dev.*`/`usr.*`) require manifest claim. E2200 `MissingCapabilityClaim` + E2201 `SelfContradictoryCapability`. `std.*`/`core.*` ambient (skip); `khi.*`/`self.*`/`super.*` intra-pkg (skip).
