...
- [~] **jit.4.agg.1c** — **boxed CORE opcodes** (data-driven per §9 re-audit). Done so far:
  ...
  - [ ] **1c-iii (next, dominant blocker `call`=2059)**: boxed `CallLocal`/`CallCrossModule`/`WitnessCall`. **Same-mode boxed→boxed** = pass ptr args, ptr result (trivial, safe — add a CallLocal arm to `translate_boxed_instruction`). **Cross-mode** (boxed↔unboxed) needs arg-unbox/result-box marshaling — design carefully (the guard currently tiers these down). Also need **branches in boxed mode** (Br/BrIf/BrTrilean — cond is a boxed Trilean ptr → an unbox-to-i8 shim `__triet_trilean_tag` then `brif`) + **Phi** (block params, agg.4 §4) since boxed functions are multi-block. Re-order by what the audit shows blocking most.
- [ ] **jit.4.agg.2** — §1 enum ops (`EnumNew`/`EnumTag`/`EnumPayload`) + Outcome ops (`OutcomeDiscriminant`/wrap/unwrap). ~764 enum + 60 Outcome (post-agg.1b counts).
