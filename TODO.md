...
- [ ] **v0.10.0.1** — [ADR-0032 NEW] Builtin shim ABI design — locks 5 constraints from ADR-0030 §12.2: (1) `RuntimeValue` ABI representation choice (boxed vs specialized vs hybrid), (2) lifetime management (`Rc::into_raw` + `drop_arc` pattern), (3) capability gate enforcement (per-builtin runtime check), (4) panic → `VmError` propagation (Cranelift trap vs `extern "C-unwind"`), (5) `unsafe_code` policy override scope.
