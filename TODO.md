...
- [x] **v0.10.x.jit.2b-iii** — Cliff shims (or defer v0.11): varargs (`FStringConcat`/`TextConcat` — fixed-arity ABI doesn't fit, needs array-ptr+len or boxed-args-Vector ABI; ADR-0032 §1 flagged "Mixed/unresolved") + file I/O ×5 (`ReadFile`/`WriteFile`/`WriteFileBytes`/`FileExists`/`ReadDirRecursive` — side-effects, non-deterministic parity, capability-gated, `ReadDirRecursive` returns `Vector<Tuple>`). Full 43-shim parity matrix (§7.2) + proptest fuzz (§7.3). Evaluate at the time: ship or clean-defer to v0.11 (they tier-down to VM = correct, just not JIT-accelerated). — `2de4d54` (+4 tests; file I/O ×5 are CLEAN delegating shims — not a cliff; **36/43 builtins JIT'd**. coverage matrix asserts 36 covered + 4 tier-down. **Varargs FStringConcat/TextConcat DEFERRED v0.11** — genuine ABI cliff (array-ptr+len), ADR-0032 addendum. raw_thread ×2 not JIT-able (VM registry). proptest fuzz redundant via delegation)
- [~] **v0.10.x.jit.3** — AOT cache via `cranelift-object` per ADR-0033. **DEFERRED v0.11** (author sign-off 2026-05-31, ADR-0033 Addendum). Path-A loader is a relocating-ELF-loader cliff: no turnkey Rust crate loads a cranelift-object `.o` + symbol resolver → executable; hand-rolling relocation patching (`R_X86_64_*`) is the most unsafe-heavy/mem-corruption-risk code in the project. v0.10 already delivered the JIT shim layer (the bulk of the JIT win); AOT cache defers to a dedicated v0.11 phase. v0.9 in-process per-process JIT (re-compile each run) stays v0.10 behavior.
- [~] **v0.10.x.jit.4** — Bootstrap gate lift + perf bench per ADR-0030 §14. **DEFERRED v0.11** — depends on jit.3 warm cache (3000-fn self-host × cold JIT is prohibitive without it). Lift `bootstrap_loop.rs::stage2_eq_stage3_main_tri_byte_identical` from `#[ignore]` + `criterion` ≥10× bench land in v0.11 once jit.3's AOT cache makes warm-cache bootstrap < 10 min feasible. Gate stays `#[ignore]` (as since v0.7).
...
### v0.11 backlog (deferred from v0.10)

- **JIT AOT cache (jit.3) + bootstrap gate lift / perf bench (jit.4)** per ADR-0033 + ADR-0030 §14. The relocating-ELF-loader cliff (ADR-0033 Addendum 2026-05-31) — design locked, implementation is a dedicated v0.11 phase. jit.4 (Stage 2≡3 byte-identical `#[ignore]` lift + ≥10× criterion bench) chains on jit.3's warm cache.
