...
- **REMAINING BLOCKERS after unreachable (audit @ 59.2%, 1683 tests):** `boxed` 1157 — now dominated by **boxed `String` constants** (~400+: `boxed constant String(...)` = **agg.3b** in-process `DataDescription`, safe) + **boxed `CallBuiltin(assert)`** (~150: `%N = call builtin.assert` boxed-mode defer — needs boxed-builtin dispatch ABI, Opus design). Plus `Constant` 131 (unboxed String/Null const) + `call` 122 + `cross-mode` 81 (unboxed→boxed composite args = **cross-call.c**: 30 Opaque + 16 String + 16 Vector) + `CallBuiltin(assert)` arity 10 + `φ` (unboxed phi).
