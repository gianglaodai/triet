...
Mốc hiện tại: HEAD `71831a9` (test) (chờ O ký + push). Gate `0·0·254·0`. **⚰️ Trục B (Heap-in-Aggregate) — nhát 1c: ASSIGN-MOVE `let q = p` ĐÓNG (chờ ký).** ADR-0066 KCN-2 assign-move — giết pseudo-copy: heap-struct `let q = p` từ alias (q,p chung local, semantics Copy SAI) → true-move (q,p tách, p tombstone), FREE_COUNT==1. **LOWER-ONLY (JIT 0 dòng** — tái dùng 1b aggregate byte-copy + Deinit struct-walk): **(D1)** is_move_binding `is_copy(None)`→`ctx_is_copy(&ty,c)` (clone type lách borrow) · **(D2)** emit `Deinit(v)` LIỀN SAU move-Assign (ATOMIC cùng BB). 1 răng đỏ route-lower R1-assign→2 (double-free). Fixture 259 + counting test `assign_move_struct_frees_once`. Plain-scalar-struct giữ alias (ctx_is_copy=true). **Nhát 1b (trước):** arg-move qua boundary (A copy_base_addr unify + B 6 to_zero ctx_is_copy + C Deinit walk); 258 + 3 răng. **Nhát 1a:** M-1 sizing + M-2 B8-relax + KCN-1 drop-glue + STEP 4 fat-store; 256/257 + 3 unit teeth. **Trước đó:** ADR-0065 §12.8/§12.7 Chuỗi Nullable Aggregate Trục A HOÀN TẤT. Kế: borrowck use-after-move (G: JIT tombstone đủ, ptr=0→SIGSEGV sạch — chưa wire E2420 cho heap-struct partial) · Vector/HashMap field (cùng mẫu) · Lát 2 nested/recursive + enum-payload heap + partial-move + field-reassign.
