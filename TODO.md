...
- [ ] **v0.9.x.atomic.7** — Demo runtime exercise. Phương án A locked 2026-05-30 (author "chậm mà chắc"): ship borrow expression syntax + E2420 enforcement minimum; NLL/lifetime/multi-thread defer v0.10 per ADR-0031 §10 backlog. Split into 5 sub-tasks:
  ...
  - [ ] **v0.9.x.atomic.7b** — Rust impl borrow expression syntax: `Expr::Borrow { form, operand }` AST variant + parser prefix rule per ADR-0031 §3 precedence + typecheck Type::Reference emission per §4 + lowerer passthrough per §5. Per-crate tests (parser × form × operand-kind; typecheck per form; lowerer passthrough proof).
