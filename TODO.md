...
- [ ] **v0.7.5.5b** — Full pattern grammar (`crates/triet-parser/src/pattern.rs` port, ~716 Rust LOC)
  - Expands the v0.7.5.3 minimal `Pattern` (Wildcard / Identifier) into the full grammar. Adds Literal (Integer / Ternary / String / Trilean) / Or / Range / Null / EnumVariant / OutcomeArm variants. Parses negative integer literals (`-5`, `-9_tryte`). Or-patterns at top level (`a | b | c`); sub-patterns use `parse_pattern_no_or` to keep `|` from binding across structural boundaries. Range patterns via `..` / `..=`. Enum variant patterns `Variant(payload)`. Outcome arm patterns `~+` / `~0` / `~-` per ADR-0020 §5.
