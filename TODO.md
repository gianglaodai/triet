...
2. **`mir_lower.rs:3730` PANIC thay vì `Err` khi caller/callee lệch `is_fat_ret`.** O poison chốt #3 →
   `panicked at mir_lower.rs:3730:43: index out of bounds: the len is 0 but the index is 0` — index `inst_results`
   không kiểm trước. **Trái Track B rule #1 ("compiler never panics on user input")**. Hiện KHÔNG reachable từ
   source hợp lệ (chỉ nổ dưới poison), nhưng là **bom chờ**: bất kỳ mismatch shape tương lai nào cũng panic thay vì
   diagnostic sạch. Hạ tầng phòng thủ = §14.7 (ba bản sao đồng bộ) + INV-Enum-shape; cả hai là *phòng*, không phải *đỡ*.
