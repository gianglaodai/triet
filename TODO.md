...
- [~] **jit.4.agg.cross-call** — cross-mode call marshaling.
  ...
  - **KEY FINDING (refcount aliasing — blocks composite + the big lever):** an unboxed callee returning a borrowed param (`id(s)=s`) does NOT bump the refcount, so passing composites across the boundary double-frees (caught by a `malloc(): unaligned tcache` abort in test). The SAME latent aliasing affects **same-mode boxed identity-passthrough** (a latent double-free in agg.1c-iii, not yet triggered by any test/the self-host compiler). The 1257 cross-mode blockers are mostly composite/`Unit` boundaries (AST nodes = structs = `TypeTag::Unit`), so scalar-only marshaling only reaches ~20. **The real lever needs a clone-on-return refcount discipline** (a `__triet_clone_arc` bump when a function returns a borrowed value) — a careful slice that also fixes the latent same-mode bug. DESIGN CAREFULLY (ADR note).
  - [ ] **cross-call.b (unboxed→boxed, scalar)**: symmetric in `translate_call` (box scalar args → temp boxes dropped after call, unbox scalar result). Smaller lever; do after clone-on-return if it helps.
