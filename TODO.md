...
- [ ] **v0.7.5.6b** — `parser_differential` NDJSON byte-diff gate (closes v0.7.5 umbrella per ADR-0019 §A7.5)
  - Mirrors v0.7.4.4 `lexer_differential` shape (~547 LOC test). Adds Triết-side `dump_program_ndjson(source: String) -> String` recursive AST walker emitting one JSON line per Item / nested node, Rust-side mirror over `triet-syntax` producing the same shape, corpus covering every Item / Stmt / Expr / Pattern / TypeExpr variant + realistic `examples/` files. NDJSON is a transient bridge format (ADR-0019 §A2) — dropped at v0.7.9 when Triết-side data flows in-memory.
