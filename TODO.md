...
Mốc hiện tại: HEAD `03018ba` (test) (chờ O ký + push). Gate `0·0·252·0`. **⚰️ Trục B (Heap-in-Aggregate) — Rào B8 THỦNG LẦN ĐẦU.** ADR-0066 Lát 1 nhát 1a ĐÓNG (chờ ký) — FLAT `struct{name:String}` construct + drop, KHÔNG move: **M-1** heap-leaf field sizing (String=24/Vec=8/HMap=8) · **M-2** B8-relax gate trên `field_decl_ty` (direct heap leaf ALLOW; transitive Struct/Enum + Nullable(heap) REFUSE; ctx_is_copy đệ quy layout-maps) · **KCN-1** inline per-struct static drop-glue (emit_heap_free_at walk layout, zero runtime overhead) · **STEP 4** fat-store (String field projected dest copy len@+8/cap@+16 — hết pass-by-luck UB). 4 răng đỏ độc lập (R-cap cap=5 thật / R-leak count 0 / R2 1<2 / R3 257 transitive lọt). Fixtures 256/257 + 3 unit teeth. **Trước đó:** ADR-0065 §12.8 `~+` Nullable-Present Unify + §12.7 Nested Nullable Aggregate — Chuỗi Nullable Aggregate HOÀN TẤT TRỌN BỘ Trục A. Kế nhát 1a tiếp: **whole-struct MOVE qua boundary** (KCN-2 copy-then-tombstone, by-pointer ABI + Deinit) · Vector/HashMap field (cùng mẫu) · Lát 2 nested/recursive + enum-payload heap + partial-move.
