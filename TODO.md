...
- [ ] **jit.4.agg.1c** — **boxed CORE opcodes** (data-driven next per §9 re-audit: after agg.1b the binding blocker is the 2545 "other" bucket = boxed Const/Add/Sub/Mul/Neg/Eq..Ge/Br/BrIf/BrTrilean/CallLocal in boxed functions, NOT enum). Each delegates to a `pub` VM helper (extract arithmetic/comparison/logic from the VM loop) + shim, except Br*/Ret which are control flow (cond unbox via a `trilean_tag` shim then `brif`). `Const` → a box shim per constant kind. `CallLocal` boxed→boxed (cross-mode marshaling deferred). **Phi + multi-block-with-shims (agg.4 §4/§5) likely needed here too** — boxed functions are multi-block; re-order as the audit demands.
