...
- **REMAINING BLOCKERS after opaque (audit @ 46.2%, HEAD `1240f35`):** `boxed` 1027 (mostly `unreachable` 644 + boxed `CallBuiltin` assert/println — **biggest lever now**) + `Constant`/String-const 131+127 (= **agg.3b** below, safe in-process `DataDescription`) + `call` 121 + `cross-mode` 81 (unboxed→boxed composite args = **cross-call.c**) + `φ` 52 (unboxed phi).
