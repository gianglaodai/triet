...
### 🎯 MẶT TRẬN KẾ — **Giang CHỐT 2026-07-27(f)**: `WO-Param-Aggregate-CopyIn`

**Quyết định của tác giả (ghi để phiên sau khỏi hỏi lại):** nổ súng vào
`WO-Param-Aggregate-CopyIn` (quả bom SIGSEGV 139 param-alias, xem nợ #2 mục
WO-Aggregate-Move-Tombstone), **theo đúng thứ tự đã chứng minh hiệu quả:**

> **recon-trước → trình bản đồ → G duyệt → soạn WO**

⚠️ **Ràng buộc Giang+O chốt kèm:** cơ chế gốc rễ ("JIT prologue để `Variable`
của param alias thẳng bộ nhớ caller thay vì copy-in") **là chẩn đoán của D, O
CHƯA verify độc lập**. O mới xác nhận **triệu chứng** (139 trên worktree sạch
`04cb5d3`) và **tính pre-existing**. Phiên sau **PHẢI đo lại cơ chế bằng MIR/JIT
dump**, KHÔNG chép khung của G hay của D vào WO — đúng bài học phiên này (nhãn
"policy-hole" sống 8 ngày vì được **suy loại** chứ không được **đo**; tên chiến
dịch "widening" phải đổi sau khi đo lộ ra `a = p` thường cũng nổ).

⛔ **CẤM** chuyển sang chiến dịch tính năng (`.drain()` aggregate key/value ·
Deep-Clone · N1 · ADR-0088 Lane B) trước khi đóng quả bom này — G sắc lệnh, Tôn
chỉ 3: SOUNDNESS TRƯỚC SYNTAX, vá crash trước mở feature.
