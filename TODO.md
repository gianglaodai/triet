...
- [ ] **v0.9.x.atomic.7** — Demo runtime exercise. Phương án A locked 2026-05-30 (author "chậm mà chắc"): ship borrow expression syntax + E2420 enforcement minimum; NLL/lifetime/multi-thread defer v0.10 per ADR-0031 §10 backlog. Split into 5 sub-tasks:
  - [x] **v0.9.x.atomic.7a** — [ADR-0031](docs/decisions/0031-borrow-expression-syntax.md) Borrow Expression Syntax Locked. Prefix `&FORM operand` (all 5 forms; bare `&` clarified non-existent), operand IDENT + field-access only (index `vec[i]` deferred — syntax doesn't exist in Triết), prefix unary precedence tier, lowerer passthrough, self-host Layer A lockstep port per ADR-0029 §3. **E2420 UseAfterMove SHIPS v0.9** (.7d); NLL/lifetime defer v0.10 per §10 backlog — `86e59b8` (draft) + lock pending.
  - [x] **v0.9.x.atomic.7b** — Rust impl borrow expression syntax. 7 files / +427 lines: `Expr::Borrow { form, operand }` AST variant (triet-syntax) + `parse_borrow`/`parse_borrow_form`/`parse_borrow_operand` parser helpers with prefix dispatch (triet-parser) + `check_borrow` typecheck emitting `Type::Reference(form, T)` with defensive borrow-of-borrow refusal (triet-typecheck) + passthrough lowerer (triet-ir) + passthrough interpreter (triet-interpreter). Operand grammar IDENT + field-access chain per ADR-0031 §2. Pratt-postfix interaction: `&+ f()` parses as `Call(Borrow(f))` with typecheck NotCallable refusal (clean UX for §10.3-deferred lifetime-extension form). Tests +19 (10 parser per form × operand-kind + precedence; 9 typecheck per form + form mismatch + NotCallable refusal). Lowerer/interpreter passthrough proof deferred to .7e e2e demo. 1474 → 1493 — `d2ab072`.
  - [x] **v0.9.x.atomic.7c** — Self-host Layer A port: `compiler/parser/parser.tri` (+153 lines). `BorrowExprPayload` struct + `Expr::BorrowExpr` variant + `parse_borrow_expr` helper reusing existing `try_parse_reference_prefix` + recursive `parse_borrow_operand_postfix` field-access chain + `format_expr` render arm. 7 new `assert_parse(...)` cases in `parser.tri::main()` exercising 5 forms + field-access operand + nested chain via `parser_expr_smoke` bootstrap test. Found self-host typechecker forward-reference limitation — `BorrowExprPayload` placed after `ReferenceForm` declaration. release-check.sh symmetry gate note: Expr is 20/56-variant subset (not 1:1), Layer A maintained by inspection. Stage 2 ≡ Stage 3 byte-identical gate stays green (compiler/.tri unchanged usage). 1493 tests passing, release-check clean — `9e0e10c`.
  - [x] **v0.9.x.atomic.7d** — E2420 `UseAfterMove` real fires per ADR-0025 §5.1 + ADR-0031 §4 Phương án A. 3 files / +414 lines in `triet-typecheck`: `MoveState { Alive, Moved { at } }` enum + `move_states: HashMap<String, MoveState>` field on Checker (reset per function); helpers `mark_moved` / `check_used` / `extract_base_identifier` / `snapshot_moves` / `join_moves` (any-branch-moves over-strict per §4). Hooks: `check_function` save/restore + params Alive, `Stmt::Let` declare Alive, `Stmt::While`/`Stmt::Loop`/`Stmt::For` snapshot-then-join body, `Expr::Identifier` `check_used`, `check_borrow` `mark_moved` (only `&+`/`&+ mutable` consume — `&0`/`&-` do not), `check_if`/`check_match` per-branch snapshot+join. **Forward-compat guarantee:** any v0.9-passing program continues passing v0.10 NLL. Tests +12: 7 positive (incl. demo canonical double-`&+`, field-access conservative move, if-branch + loop-body joins), 5 negative (each non-owning form + single-use + both-branches-preserve). NLL/lifetime/`&-`-upgrade defer v0.10 per §10.1. Self-host port deferred Layer B per §10.6. 1493 → 1505 — `bb92aa5`.
  - [x] **v0.9.x.atomic.7e** — `atomic_counter` demo single-call upgrade per ADR-0031 §9 Phương án A. Demo (~30 net lines): `let counter = new(0); let prev = spawn_worker(&+ counter); println(f"...{prev}"); prev`. `main` returns `Integer` for test assertion. Multi-worker pattern + interpreter parity gap documented as v0.10 backlog in source + ADR-0031 §10.7 NEW. New `crates/triet-cli/tests/atomic_counter_e2e.rs`: in-process VM execution from real demo source on disk — asserts result == `Integer(0)` proving full pipeline (parser borrow → typecheck → lowerer passthrough → VM dispatch `AtomicNew` + `AtomicFetchAdd` via `path_to_builtin` intercept). Discovered interpreter parity gap (`triet-interpreter` doesn't intercept `sys.atomic.*` paths → stack overflow on placeholder body recursion); demo is VM-only per outcome_propagate.tri precedent. Output: `Atomic counter previous: 0` confirms println + fetch_add return semantics. 1505 → 1506 — `8296895`.
...
### v0.9.x.borrow — DELETED (folded into .7d per ADR-0031 §4)

**Decision 2026-05-30 (Phương án A):** v0.9 ships only **E2420 UseAfterMove** (in `.7d`). Full borrow checker enforcement (E2440 NLL CFG live-range, E2400 lifetime elision 3 rules, E2403 `&-` upgrade) defers v0.10 per ADR-0031 §10.1 backlog. Reason: ADR-0025 locked "full NLL enforcement defer v0.9 (cần real-world Triết corpus)"; v0.9 corpus too small for NLL design validation. v0.9 .7e demo single-call pattern doesn't exercise NLL/lifetime — E2420 alone is enough.
