...
- [ ] **v0.9.x.atomic.7** — Demo runtime exercise. Phương án A locked 2026-05-30 (author "chậm mà chắc"): ship borrow expression syntax + E2420 enforcement minimum; NLL/lifetime/multi-thread defer v0.10 per ADR-0031 §10 backlog. Split into 5 sub-tasks:
  ...
  - [ ] **v0.9.x.atomic.7e** — `atomic_counter` demo single-call upgrade per ADR-0031 §9 Phương án A: `let counter = new(0); let prev = fetch_add(&+ counter, 1, Synchronized); println(prev)`. Multi-worker pattern documented as v0.10 backlog comment in source. E2E test asserts `Counter previous: 0`. Exercises ADR-0028 §9 test gate item 2 (round-trip correctness on single-thread VM).
