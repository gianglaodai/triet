...
- [~] **jit.4.agg.3** — Nullable ops + String/Null constants.
  ...
  - [ ] **3b-iii (small follow-up)**: UNBOXED `Constant::String`/`Null` (`materialize_constant`, codegen.rs:2147) — the residual `Constant` 131 first-blocker. Same `__triet_box_string` mechanism (composites are i64 ptrs in unboxed mode too); needs `module` threaded into the unboxed const path. Small lever vs boxed; measure after.
...
- **REMAINING BLOCKERS after agg.3b-i (audit @ 87.6%, 1685 tests):** the entire `boxed` first-blocker category (1157) is GONE — boxed String const was masking it. Now: `call` 194 + `Constant` 131 (unboxed String/Null const = **3b-iii** above) + `cross-mode` 89 (unboxed→boxed composite args = **cross-call.c**: 30 Opaque + 16 String + 16 Vector) + `φ` 52 (unboxed phi) + `CallBuiltin(assert)` arity 10. Bootstrap-gate-lift (jit.4.gate) is now within reach — ~12.4% residual is mostly unboxed-path (call/Constant/φ) + the small cross-mode composite tail.
