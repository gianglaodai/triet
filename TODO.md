...
> ⚠️ **Mốc thật, đo 2026-08-25 tại `7521e69`: Gate `0·clean·0·581·0`, exit 0, CLEAN, 581 fixture files.**
> Dòng "Mốc hiện tại" ngay dưới đây đứng im ở `522` (2026-07-27) — nó là mốc của lát ADR-0089
> §AMEND-2, KHÔNG phải trạng thái hôm nay. Sự thật sống: [`spec/PROJECT_KNOWLEDGE.md` §Maturity](spec/PROJECT_KNOWLEDGE.md).
...
> **`-T` chỉ khai báo ở GLOBAL và phải tính được LÚC COMPILE** (`.rodata`), **không** phải biểu thức
> runtime như chốt lúc đầu phiên. Lý do: biểu thức thì số lượng = số lần chạy ⇒ `-T` trong hàm gọi
> trong vòng lặp = phình RAM không chặn, không tín hiệu. Giang bác cả hai cách giảm nhẹ của O
> (ngân sách arena + lint) để **xoá hẳn chế độ hỏng**.
> **Được:** rò rỉ bất khả thi · safe publication tự tan (loader ghi `.rodata` trước `main`) ·
> read-only do OS, khỏi `mprotect` · nhanh nhất (RIP-relative, **0 lần load con trỏ**).
> **Mất:** `-T` không chứa được dữ liệu runtime ⇒ config lúc khởi động · interned string · bảng ký hiệu
> **rơi hết** ⇒ **lỗ `Rc`/`Arc` chỉ lấp một nửa**. Và **thứ tự đảo**: `-T` giờ chặn sau const-eval +
> phát mã `.rodata` (**đo: 0 dòng hiện có**) ⇒ **`+T` là đường DUY NHẤT tới kiểu đệ quy và self-hosting.**
> **Lời giải cho phần mất — chia vai, không chồng lấn:** `-T` = hằng biết lúc biên dịch ·
> **Arena + generational index** = bất tử nhưng dựng lúc chạy. ⇒ **Mục K TAN** (nó tồn tại chỉ vì hai
> thứ này giẫm chân nhau). **Mục M co lại** (comptime-`-T` không thay được đường `&+`-frozen runtime
> ⇒ refcount §3.2 chưa về hưu). **Mục O phần ④ tháo ngòi** (không còn lúc dựng thì không còn bài toán
> công bố). Mục "ngân sách arena cho `-T`" **không sinh ra nữa**.
> ➕ **Mục R mới:** const-eval **tối thiểu** (literal · số học trên hằng · composite toàn hằng —
> **KHÔNG** cần comptime cỡ Zig, đừng để bị đẩy sang hậu-self-hosting oan) + phát mã `.rodata`.
> Đo: `const_eval`/`comptime` **0 hit** trong `crates/`; `.rodata` chỉ có 1 dòng ghi chú AOT ở
> `crates/triet-jit/src/mir_lower.rs:3056`.
>
> 🔜 **CÂU HỎI MỞ ĐẦU PHIÊN SAU (Giang):** **bỏ hẳn `Rc` được không? Và `Arc` có phải chỉ là câu
> chuyện từ self-hosting trở đi, khi mới thật sự có đa luồng?** Giang tự đánh giá: **`Arc` là quyết
> định sớm của chính anh** — lo đa luồng khi chưa self-host. Mở lại từ đầu: sau khi `-T` lo hằng
> biên dịch và Arena lo bất tử-dựng-lúc-chạy, thì **còn gì thật sự cần sở hữu chia sẻ?**
>
> Đây là **danh sách đóng**. Trả hết mục A→E rồi mới mở campaign `+T`. Thiết kế đã chốt ở
> [`.claude/memory/idea_ternary_placement_syntax.md`](.claude/memory/idea_ternary_placement_syntax.md)
> + [`idea_reference_forms_and_refcount_debt.md`](.claude/memory/idea_reference_forms_and_refcount_debt.md).

## Phần 1 — NỢ CODE, chặn `+T` (phải trả, theo đúng thứ tự này)

| # | Nợ | Vì sao chặn Box | Ghi ở |
|---|---|---|---|
| **A** | **`WO-Harness-Subprocess-Isolation`** (nợ P3) — harness chạy in-process, một SIGILL giết mọi assertion phía sau | Làm `+T` sẽ đẻ SIGSEGV/SIGILL liên tục (kiểu đệ quy, drop glue đệ quy). **Harness nói dối thì mọi verify sau đó vô giá trị** ⇒ phải trả TRƯỚC, không phải vì nó to nhất mà vì nó là cái cân | `campaign_reference_operand_eq_refuse.md:98` |
| **B** | **`WO-Literal-Temp-Drop-Leak`** (nợ P2) — `if s == "hi"` ⇒ temp `_1 = const "hi"` không bao giờ drop; 2 alloc 1 free | Gốc ở temporary rvalue trong lowerer. **`+T{...}` CHÍNH LÀ một rvalue temporary sở hữu heap** ⇒ gốc chưa vá thì mọi `+T{}` dùng làm temp đều rò. G xếp ưu tiên cao nhất (rò bộ nhớ) | `campaign_reference_operand_eq_refuse.md:95` |
| **C** | **MÙ LỚP BỌC** — `&0 T?` lách E1033 (nợ ghi nhận #5) **+** `Reference{Reference{…}}` chưa đo | **Cùng một bệnh**: lớp bọc ngoài làm mù `matches!` kiểm hình dạng bên trong. **`+T` là lớp bọc THỨ BA** — mọi chỗ đang mù với `Reference` sẽ mù với `+T` y hệt. Đây là hình dạng đã đẻ ra **5 chiến dịch "sai âm thầm, exit 0"**. 🔑 Chữa bằng **một hàm bóc-lớp-bọc dùng chung**, không vá từng chỗ ⇒ trả cho `Reference` + `T?` và **trả trước cho `+T`** | `campaign_reference_operand_eq_refuse.md:107-113` |
| **D** | **Enum sret có payload String — CHƯA ĐO** (E2423 chặn probe) | `enum` payload `+T` là con đường đa hình 90% đã chốt ở `ADR-0061:123`. Đường đó đang có một khúc chưa ai soi | `campaign_reference_operand_eq_refuse.md:115` |
| **E** | **ADR gộp P4 + P5** — `String <` (SPEC nói có, compiler E1004) và **null-equality chưa có đặc tả** (`Integer? == ~0` → `0`, không bao giờ `Unknown`) | Chặn **một nửa**: `+T?` thừa hưởng nguyên xi cái gì `Nullable` đang làm. Chốt SPEC trước, không thì đẻ thêm một biến thể của cùng lỗ hổng. **G đã xếp: cần ADR, không phải WO vá tạm.** Đã cắm `⚠️ SPEC GAP` tại `SPEC.md` §2.5 và §4 | `campaign_reference_operand_eq_refuse.md:99-106` |

**KHÔNG chặn** (đừng lôi vào): Outcome 2-reg ABI · multi-value return · native layout · AOT cache · self-host.

## Phần 2 — NỢ THIẾT KẾ, 0 dòng code, trả bằng ADR/văn bản

| # | Việc | Ghi chú |
|---|---|---|
| **F** | **Viết ADR-0090 — trục vị trí `+T`/`T`/`-T`** | Số trống kế tiếp là **0090** (0068 và 0073-0075 là lỗ, ĐỪNG lấp). Trục phải gọi tên là **nghĩa vụ giải phóng**, không phải "3 vùng nhớ" |
| **G** | **Đóng đinh `&+` = strong owner** | G đã viết lại nghĩa thành "tham chiếu bất tử/đọc toàn cục" **3 lần trong 3 tin nhắn**, trái `ADR-0022:58` (LOCKED) + schema `StrongFrozen`. `-T` làm nghĩa trôi đó thành thừa |
| **H** | **AMEND `&-` = dòng chảy ra ngoài, xoá `Weak`** | Sửa `docs/decisions/0022-trit-balanced-ownership.md:62` + biến schema `WeakObserver` (qua `spec/schema/triet-schema.yaml`, **cấm sửa tay file sinh**) |
| **I** | **Luật mới: struct có `&-` trỏ lên chủ thì CẤM move ra khỏi chủ** | Không có luật này thì ví dụ AST-parent **không an toàn**: `pop_front` (`crates/triet-typecheck/src/env.rs:384`, đã ship) móc con ra khỏi `children` ⇒ dangling. Đây là việc của `Pin`, làm ở tầng kiểu |
| **J** | **Luật nới lỏng một chiều `&0 -T` dùng được ở chỗ chờ `&0 T`** | Thứ giữ cho 90% chữ ký hàm không phải nhắc tới vị trí |
...
Mốc hiện tại: Gate `0·clean·0·522·0`, synced sạch (2026-07-27). **🏁 ADR-0089 §AMEND-2 — `HashMap.drain()` LANDED qua PA-2 DESTRUCTURING-ONLY DESUGAR (O verify máu 3 mũi poison + G ký + Giang ký 2026-07-27)** (code `816a729` [D], docs [O co-sign]). Giang chốt mở "Tuple lowering để xử lý HashMap.drain()"; **O recon BÁC luôn cả khung của chính nhãn đó**: nhãn defer cũ ghi 2 bức tường (Tuple + enumerate-shim) — đo lại thì **bức tường 2 KHÔNG phải bức tường** (drop-glue `mir_lower.rs:1940`/`:2038` + rehash `:6583` **ĐÃ walk key-less toàn bộ `cap` lọc `state==1`** từ lâu; cơ chế CÓ, chỉ chưa expose), còn **bức tường 1 thì KHÔNG nên phá mà nên ĐI VÒNG**. 💀 **PA-1 (Tuple hạng nhất) BỊ G ĐẠP CHẾT:** `MirType::` bị match tại **729 site** (riêng `mir_lower.rs` 29 match exhaustive) ⇒ thêm 1 variant = **tự gieo lại họ bug "match exact QUÊN variant"** vừa tốn trọn chiến dịch để quét (6 thành viên, 2 nằm TRONG lưới an toàn), lại chạm B-γ multi-reg (defer vô hạn) + kề B-β sub-8B (đã đạp chết). 🔑 **Câu hỏi kiến trúc quyết định: `for (k,v)` cần HAI BIẾN trong thân vòng, KHÔNG cần một GIÁ TRỊ tuple** — PA-1 xây kiểu hạng nhất chỉ để lập tức phá ra làm hai, trả 729 site cho một trung gian không ai giữ. **PA-2 = 0 variant mới, 0-hit trên 729 site.** 🔒 **BẤT BIẾN TỐI CAO: Tuple SỐNG ở front, CHẾT tại lower** — `MirType::Tuple`=**0** toàn backend (O tự verify). ⚠️ **Tiêu chí kiểm SAI đã ăn đòn: `grep -c Tuple` trần KHÔNG phải tiêu chí** (lowerer BẮT BUỘC match `triet_syntax::Pattern::Tuple:2036` để destructure = chính thiết kế) — **D bác tiêu chí proxy thô của O bằng số đo, D ĐÚNG**; tiêu chí duy nhất đúng = `MirType::Tuple`=0. **3 điểm chạm:** typecheck `check.rs:829-845` (tuple-2 ∧ key_ok ∧ value_ok → `Type::Tuple([K,V])` bind qua `bind_pattern:1097`) · lower `lib.rs:2031+` (destructure thành **2 local riêng** `_key`/`_val`, CFG mirror Slice 2b) · JIT shim mới `__triet_hashmap_drain_next` `mir_lower.rs:7005+`. **🔑 Chuỗi 4 bước move-out (mirror `__triet_hashmap_remove:6824`):** surface K+V ra out-ptr → **zero key-cell** → **`state→2`** → **`len--`** → trả `idx+1`. **MỘT cờ `state` đóng CẢ BA tử huyệt bộ nhớ** (drop-glue chỉ walk `state==1`): ① move-out sound (tombstone miễn double-free) ② break-mid (đã-drain `2` bỏ qua + còn-lại `1` drop-glue dọn nốt) ③ container-survives (`len--` ⇒ drain trọn `len==0`, re-insert hợp lệ). **Cursor O(N) KHÔNG rescan O(N²)** (`cap=1000,len=10`: **1000** vs **10.000** lượt đọc state); sound-stop `while idx<cap` kiểm ĐK **trước** khi đọc byte ⇒ `cap==0` an toàn (fixture 525 canh). **🔑 QUY ƯỚC SENTINEL (G bắt ghi):** cursor-stop = **`-1`**, KHÁC `NULL_SENTINEL`=`i64::MIN` (giá trị vắng mặt/nullable) — miền cursor luôn `≥0`; **CẤM trộn hai khái niệm**. **Fence lát 1:** MỞ K∈{scalar,String} V∈{scalar,String,Vector,HashMap}; REFUSE **E1054** (pattern≠tuple-2 · tuple-3 · aggregate key/value · `V=Nullable`); `m.drain()` ngoài for-guard → **E1015** (tiền lệ Vector 491). ⚠️ **Nợ chẩn đoán: E1054 nay mang 4 NGHĨA**, ca pattern-shape (527/528) vẫn in `key`/`value` dù nguyên nhân là hình pattern — G tạm chấp nhận lát 1, tách sau nếu siết "một E-code một hợp đồng". **🦷 11 fixture 520-530 + `hashmap_drain_counting.rs` (7 test, DEDUP CON TRỎ: assert `count==N` VÀ `dup==0`** — FREE-count đơn thuần mù trước double-free: 3 free có thể là 3 object HOẶC 2+1 trùng). **🩸 O verify máu 3 mũi ĐỘC LẬP:** **P1** `state→2`→`1u8` ⇒ `drain_full` **9 vs 6** · `break_mid` **10 vs 8** con trỏ lặp = **double-free THẬT** (bãi mìn move-out đồng thời heap-key×heap-value — lần đầu trong lịch sử dự án) · **P2** bỏ `len--` ⇒ `drain_full_leaves_len_exactly_zero` **3 vs 0** · **P3** guard fail-**open** ⇒ 527-530 đỏ **+ fixture CŨ 510 đỏ lây**, 520-525 **không** đỏ. ⚔ **Bài học P2:** "không đỏ" phải phân định (a)/(b) bằng **đường-chạm-được** — vòng drain dừng theo `state` qua cursor **KHÔNG theo `len`** ⇒ (b) test yếu; **D báo TRUNG THỰC rồi TỰ cắm thêm răng** đọc thẳng `len(m)`, KHÔNG bịa mũi giả. ⚔ **Bài học P3: "tháo guard" = fail-OPEN không phải fail-closed** — D làm sai hướng lần đầu (`if false &&` = siết chặt hơn, chứng minh 0), tự phát hiện, sửa `if true ||`, đo lại; dưới poison đúng hướng các hình refuse **không lọt** mà bị lowerer chặn ⇒ **defense-in-depth 2 lớp** (typecheck=mã đúng, lower=fail-closed cuối), cùng kiến trúc ADR-0088 Lane A. 🩸 **O SAI 2 TIÊU CHÍ, D bác cả 2 bằng số:** (1) `grep -c Tuple` proxy thô sẽ reject chính thiết kế O ra lệnh; (2) "gate mục tiêu 530 fixtures" — nhầm **số hiệu cao nhất** (530) với **TỔNG SỐ FILE** (522), trong khi **chính O đã chạy `ls|wc -l`=511 cùng phiên**. Cùng gốc "hành động trước khi đo". **Nợ mở:** aggregate key/value drain (move-out key aggregate = ABI mới) · `V=Nullable` (drain qua out-param KHÔNG bọc Nullable nên lý thuyết an toàn hơn `Vector<T?>` — **chưa đo** ⇒ giữ refuse) · tách E1054 · **PA-1 Tuple hạng nhất VẪN BỊ BÁC**, chỉ mở khi có use-case multi-return thật. --- (lát trước) **🏁 ADR-0088 LANE A ĐÓNG — hàng rào `E1055` nested-nullable 2 tầng / 2 lớp nguồn + 9 răng cưa (O verify máu 3 mũi poison + G ký 2026-07-27)** (code `d9b659a` [D], docs [O co-sign]). Giang chốt mở ADR-0088; **O recon 20 probe BÁC khung "double-nullable = vấn đề của `get`-family"** — thân bài ADR chỉ phủ `get`/`get_ref`, còn `T??` viết TRỰC TIẾP chưa ai đo: **KHÔNG UB** (mọi đường fail-closed) **nhưng chẩn đoán VỠ — 5 mã lỗi cho CÙNG một khái niệm**, gồm 🔴 **E1190 = mã ICE "please report compiler bug"** cho input user hợp lệ cú pháp (vi phạm taxonomy ADR-0086) + message MIR **sai hướng hoàn toàn** (nói "heap-nullable/Struct?/Enum? Copy-only, [Fix] Remove the heap field" cho một `Integer??` không có heap không có struct — vi phạm ADR-0027). 🎯 **Phát hiện lớn nhất: 0 RĂNG CANH** — toàn bộ refuse sống ký sinh vào **allow-list** `is_lowerable_nullable_payload` (`triet-mir:1796`, `Nullable` không có tên ⇒ rơi ra ngoài ⇒ refuse *by default* = **may mắn cấu trúc**), grep `??` trong dòng code corpus = **0 hit** ⇒ ngày Lane B nới allow-list thì 7 đường lọt JIT cùng lúc, **câm, gate vẫn xanh** (hình dạng SPOF-một-lớp WO-SPOF-1). **HAI lớp nguồn** (WO không thể "một điểm chạm"): **Lớp A** `resolve_type` **2 BẢN SAO** `check.rs:1365`+`check_resolved.rs:597` (annotation) · **Lớp B** `check_call` sau `substitute` (`check/exprs.rs`) — `let x=pop(v)` KHÔNG có annotation nào, shape chỉ tồn tại sau substitution. ⚖ **D BÁC vị trí WO của O — ĐÚNG:** O gợi ý `env.rs:374/394/506`, D chỉ ra nơi đó `T` còn là `TypeParameter` trừu tượng, không biết bind ra gì ⇒ chuyển `check_call` = chokepoint DUY NHẤT; O poison đặc hiệu xác nhận D đặt đúng chỗ. ⚖ **O BÁC giao thức verify của G — G rút:** G lệnh "poison allow-list → 7 fixture đỏ"; O đo trước và chứng minh sẽ KHÔNG đỏ (typecheck chặn trên bọc kín tầng MIR) ⇒ giao thức 1-mũi sẽ đẩy người verify vào bẫy "poison không đỏ" rồi buộc bịa mũi giả cho nổ — G phê chuẩn **2 mũi độc lập**. 🩸 **O verify máu 3 mũi + đặc hiệu HAI CHIỀU:** tắt Lớp A→**6** đỏ (511·512·513·517·518·519), 514-516 xanh · tắt Lớp B→**3** đỏ (514·515·516), 6 kia xanh (**6+3=9, hai lớp TÁCH BẠCH, không lớp nào đội lốt lớp kia**) · nới allow-list MIR→**chỉ unit test đỏ, 0 fixture** (đúng thiết kế). Dưới mũi 1a, **517/518 lộ lại ĐÚNG ICE cũ** ⇒ guard mới chính là thứ giết E1190/E1141. **Răng chứng minh ở TẦNG HARNESS** (đổi `// ERROR:` 514→`E9999` → dòng FAIL expected/got, luật 15); restore `cp`+md5 khớp, 0 `git checkout`. ⚔ **Bẫy sai-dương D TỰ phát hiện + TỰ sửa:** message MIR nháp chứa chuỗi `"(E1055)"` → harness `.contains(code)` xanh giả dù đã gỡ guard → D bỏ chuỗi mã khỏi message runtime, chạy lại bằng harness thật. **Bất biến khắc: message runtime tầng này CẤM chứa mã lỗi tầng khác** — nếu không mọi poison xuyên tầng vô hiệu câm. **Control chống over-refuse (giữ mãi):** struct `Integer?` một tầng→`16` · insert-store→`5` · flatMap 175/212/213 xanh (`exprs.rs:361-364` không bao giờ sinh `U??`) · 465/466/467 **giữ E1051** không bị E1055 cướp. **Ranh giới E-code:** E1051=`get`-family (không đụng) · **E1055**=nested `T??` mọi vị trí khác. 🩸 **Vết O: sót hạng mục ④ Documentation Integrity của G khi soạn WO** (không giao D) — O tự thi hành, không gọi D lại. **§AMEND-1 §88A.4 ĐÍNH CHÍNH thân bài ADR-0088:** câu *"Guard KHÔNG chặn `contains`"* mô tả **hành vi không tồn tại** — đo thật `contains(m,1)` với `V=Integer?` → **E1041 NoMatchingOverload** (overload table không khai `V` generic), KHÔNG phải được cho qua; ai tưởng `contains` là workaround hợp lệ cho `HashMap<K,V?>` sẽ trượt. **⏸️ LANE B (thiết kế `T??` thật: repr 3-trạng-thái + ABI + match ergonomics + parser `??~`) HOÃN VÔ THỜI HẠN** — G: chưa có use-case đòi phân biệt "key không tồn tại" vs "giá trị lưu là null" thì là xây cầu khi chưa có sông; workaround = sentinel value / wrapper Struct có cờ `present`. --- (lát trước) **🏁 HashMap.drain() DEFER (PA-D) — E1054 fail-closed, ADR-0089 §AMEND HashMap.drain(), O verify máu + G ký 2026-07-27** (code `c001075` [D], ADR `31eea94` [O co-sign]). Recon BÁC nhãn backlog "mirror Vector.drain / bucket state-gate" — nhãn bỏ sót bức tường lớn hơn: **2 bức tường** (1) yield `(K,V)` cần Tuple lowering mà Tuple **0-hit** lower/mir/jit; (2) không có enumerate-entry shim key-less (mọi shim HashMap key-indexed). Giang cấm PA-B/C lossy → refuse RIÊNG **E1054** (`DrainHashMapUnsupported`, KHÔNG rơi E1052 chung chung), **scope drain-only** (G chốt: một E-code một hợp đồng — 471 plain `for x in m` giữ E1052, Vector.drain giữ E1053). 1 điểm chạm typecheck `check.rs:795` + variant `error.rs` E1054 + fixture 510. **O poison máu:** tắt HashMap arm → fixture 510 `expected E1054, got E1052` ĐỎ (răng tầng harness), restore byte-identical md5 `bd8c08c4…`. **Prerequisite feature thật = campaign Tuple lowering (MIR+JIT).** --- (lát trước) **🏁 PHÁO ĐÀI #6 — ADR-0089 Slice 2d (`&0 mutable Vector` drain) ĐÓNG — O verify máu + G ký 2026-07-27** (code `014442e`+`2dcc9b6`, ADR `75e5c6a`). Drain QUA mượn-độc-quyền-mutable: **container-survives** (caller giữ buffer rỗng, §2d.2) · **break-mid caller-drop soundness** (tombstone len-- để survivors 0..len cho owner drop sau, §2d.3) · **form-aware fence** (chỉ `BorrowExclusiveMutable`, T non-nullable; form khác/T?→E1053/E1051). **3 điểm chạm:** typecheck form-aware `check.rs:759` + lower Reference-unwrap `lib.rs:2373` + JIT fat-detect Reference-unwrap `mir_lower.rs:3909` (D tự bắt lỗ ngoài scope phase-1→phase-2 mở điểm chạm #3, mirror idiom `_get_copy:3967`). Fixtures 505-509 + `drain_borrow_break_mid_counting.rs` (full=3, break-mid=5). **O poison máu:** form-guard→492/507 ĐỎ · JIT unwrap→506 ĐỎ · **no-drop 2-lớp** (`is_copy(Reference)` lowerer + JIT Drop:3397) chokepoint→`Drop not supported` fail-closed. Chi tiết [[campaign_iteration_slice2d_borrow_drain]]. --- (lát trước) **🏁 PHÁO ĐÀI #5 — ADR-0089 Slice 2c (`!!` ForceUnwrap) ĐÓNG — O verify máu + G ký 2026-07-27** (code `aa500ab`, ADR `c77d674`). `!!` lower đồng cấu Elvis `?:` PA-3c identity: **trap-on-null** (`Terminator::Trap`/SIGILL) · present PA-3c identity Assign (move-out non-Copy heap-scalar → E2420 khi đọc lại, giết alias double-free) · scope scalar + heap-scalar (String?/Vector?/HashMap?) · **fence non-Copy Aggregate** `Struct?`/`Enum?` → **E1100** (defer ownership projection). 1 điểm chạm: arm `Expr::ForceUnwrap` trong `triet-lower/lib.rs:4508`. Fixtures 495/497/499-504 + subprocess trap teeth `force_unwrap_null_trap.rs` (496/498 cô lập — SIGILL trong corpus giết harness) + `force_unwrap_counting.rs` FREE==1. **O poison 2 mũi ĐỎ:** tháo Trap→trap teeth (signal 4→None) · tháo fence→corpus 501/502 (pipeline succeeded 3/7) + double-free exit 134 trên heap struct. ADR-0041 §AMEND-Slice2c (O✅/G✅). Chi tiết [[campaign_iteration_slice2c_force_unwrap]]. --- (lát trước) **🏁 ADR-0089 Slice 2b (Drain) ĐÓNG — O verify máu + G ký 2026-07-26** (`780e081`). `for item in v.drain()` tiêu thụ `Vector<T>` **move-out by-value** MỌI T (heap MỞ: `Vector<String>`/`Vector<User{String}>` — khác Slice 2a copy-refuse E1053). Zero-shim: desugar vòng `pop_front` proven (347/351/338), O(N²) correctness-first. 2 điểm chạm: typecheck `check_for_stmt` (.drain()=for-guard-ONLY→standalone E1041; ref→E1053; `Vector<T?>`→E1053; else→E1052) + lower drain-arm (pop_front+present-test+PA-3c unwrap, continue→hdr no-step). **🥇 Tombstone `len--` DOUBLY LOAD-BEARING** (O poison: tháo→full-drain HANG VÔ HẠN [điều kiện DỪNG] + break-mid double-free [survivor]). Fixtures 486-494 + teeth `drain_iter_counting.rs`. D cắt-ngang mid-verify→O verify độc lập+dọn (restore poison `mir_lower da3a0d80` KHÔNG-vào-commit + sửa docstring giả-thuyết-sai). Chi tiết [[campaign_iteration_slice2b_drain]]. **⚑ Các cờ `🚩 drain (ADR Iteration)` rải trong milestone-log CŨ dưới đây (ở note pop_front `5462c5b`, get-by-value `28f7c6f`, Slice C) = ĐÃ ĐÓNG bởi lát này — KHÔNG phải backlog mở (chống zombie).** ⚑ Giữa `36dad8e`→`780e081` còn 5 lát đã đóng ghi ở `git log`/memory (ADR-0086 Front-A `dbde2d5` · Front-B `e2db04d` · ADR-0085 SPOF `f6b569f` · WO-Robust-InstResults `3fc170f` · &0-String coverage · WO-MethodCallFatReturn `45919cd` · ADR-0084 verify + Slice 1 + Slice 2a phiên 2026-07-26) — header cũ dưới đây stale ở `462`, đọc `git log` + memory cho sự thật.

--- (milestone-log CŨ, giữ nguyên văn cho lịch sử — trạng thái sống ở dòng trên + memory) ---

Mốc cũ (stale `462`): Gate `0·clean·0·462·0`, origin/main = `36dad8e`, synced sạch. **🏁 WO-GetRefNullableRefuse ĐÓNG — O verify máu + G ký 2026-07-25.** Recon lane "get_ref V=Nullable" (nhãn coverage-lẻ) lộ **design cliff double-nullable**: `get`→`V?`; V=`T?` ⇒ `(&0 T?)?`/`T??` (outer=key-found vs inner=stored-null xung đột; silent-flatten=wrong-result). G chọn **refuse sạch + defer ADR-0088**: guard `exprs.rs` (name=="get", sau heap-violation) → **E1051** `GetContainerNullableValueUnsupported`, `return Type::Unknown`. 🩸 O poison lộ tầng MIR CŨNG vỡ nếu lọt (double bằng chứng). 4 commit `6ebef58`+`768c631`+`62087a7`+`36dad8e`, fixtures 458→462. Recon-trước-WO cứu LẦN 3 phiên này. **🏁 WO-Robust-InstResults ĐÓNG — O verify máu + G ký 2026-07-25.** Track B rule #1: 10 site `inst_results(call)[i]` panic-index → helper checked `nth_call_result(...)?` → `JitError::CallResultArityMismatch{callee,expected,got}` (struct variant). Teeth hand-built MIR poison→PANIC/fix→Err-sống, O tự re-poison site 4764 md5 `da3a0d80`. 3 commit `fc51f05`+`b270df4`+`3fc170f`. Class ICE inst_results-index khai tử. G sửa lưng O: "poison-only hôm nay = ICE ngày mai" (Tôn chỉ 3 SOUNDNESS TRƯỚC SYNTAX — vá crash trước mở feature). **🏁 WO-JIT-PRINT / ADR-0087 ĐÓNG — O verify máu + G ký 2026-07-25.** print/println stdout hoạt động lần đầu sau rewrite: **4-overload** (owned String move+free / `&0 String` borrow+no-free) × **4 extern-C shim** (`__triet_print[_ref]`/`__triet_println[_ref]` — owned arity-3 ptr/len/cap→free, ref arity-2 ptr/len→no-free, memory-responsibility hardcode vào symbol name KHÔNG cờ `is_owned`) + `emit_shim_call` **nhánh `ReturnShape::Unit`** (trả Unit local thật `ConstValue::Unit`, KHÔNG throwaway i64 — G bác) + typecheck `declare_overload` (KHÔNG `declare` — `resolve_overload` gated `lookup().is_none()`, regular binding sẽ nuốt overload). Cap **compile-time only** (VISION §capability, KHÔNG runtime `__triet_cap_check`). 6 commit: `867b0d4`(ADR G-ký) `c82487b`(emit_shim_call Unit) `6cf8dcf`(route+meta) `c5fc6f8`(env overload) `4c88805`(C-shim) `25cb2cc`(4 teeth). 🦷 Teeth subprocess fork-guard + counting-free (đếm-rồi-free-thật, dedup con trỏ): T1 println-owned FREE=1 · T2 println-ref reuse-after "x\\nx\\n" FREE=1 · T3 print no-newline · T4 routing "a\\nb\\na\\n" FREE=2. **🩸 O TỰ re-poison T1 độc lập:** `__triet_println [true]→[false]` → child crash `free(): double free detected in tcache 2` (UB THẬT glibc), restore md5 `bade48f3` khớp, xanh lại. **HAI vết recon O** (D bắt qua 2× LUẬT-4, không phải lỗi D): (a) WO sót env.rs → `&0` overload unreachable nếu không `declare`→`declare_overload` · (b) 2 test WrongArity/Mismatch cột vào builtin `print` → tách sang **user-defined fn** (giết cả lớp fragility). D khai thật main.rs (wiring ShimSymbol registry, WO O ghi nhầm chỗ) + T4 failure-mode lệch dự đoán (refuse-compile qua guard marshalling, KHÔNG leak — refuse-over-guess tốt hơn, D báo đúng thực tế quan sát). ⚖ D=Sonnet 5 subagent, 2× DỪNG đúng LUẬT-4, 0 vết bịa. 🔴 **Nợ defer (đáy sổ):** `read_line` (input) · f-string/format runtime · buffering policy (line vs unbuffered). **🏁 `&0 String` READ-BORROW COVERAGE (len/eq/concat) ĐÓNG — O+G ký 2026-07-25, `31fd2d6` (feat) + `4904c55` (docs G-state).** Vá 'mạch lở loét' `&0 String`: `len(&0 Vector/HashMap)` chạy nhưng `len(&0 String)`→E1041, `concat/eq(&0 String)`→E1003 (while `length/contains/is_empty(&0 String)` đã chạy). Gốc = typecheck overload thiếu (env.rs), KHÔNG lowering: `len`+`eq` chỉ thêm overload (JIT `bung_fields` đã có nhánh `is_reference()`); `concat` cần MỞ RỘNG JIT — class `concat_sret` (mir_lower.rs) thiếu nhánh `is_reference()` fallback, fix = **mirror nguyên văn từ `bung_fields`**, đồng nhất marshaling hai class. Phương án A (explicit overloads — G BÁC coercion ngầm). Teeth máu dedup-con-trỏ: `len` FREE=1, `eq` FREE=2, `concat` FREE=3 (2 mượn+1 result alloc), dup=0; poison 2 tầng (gỡ overload→E1041; gỡ nhánh JIT→'arg without slot'), O verify độc lập cp-snapshot. Fixtures 462/463/464. **🏁 WO-JIT-Print ĐÓNG 2026-07-25** (chi tiết ở đầu Mốc + ADR-0087, `25cb2cc`). **🏁 ADR-0085 NHỊP 2a — self-loan-exclusion M3 mutate-precheck ĐÓNG (E2440 self-conflict `clear`/`append` + `mutates_arg:Some(0)` phục hồi) — O+G ký 2026-07-24, `7c8cbf2`+`9a9cb72`. MIR-grounded, T2 hai lưỡi non-vacuous, luật 18 dính O ×3 phiên (poison cứu cả ba). Nợ Nhịp 2b (P-flag canary) treo chờ campaign sau — chi tiết ở nợ SPOF dưới.** **🏁 ADR-0085 NHỊP 1 — SPOF `builtin_shim_meta` P-exist ĐÓNG (bảng toàn phần 8-entry + cổng tồn tại `Body::verify()` discriminator `__triet_`) — O+G ký 2026-07-24, `63887e4`. Poison `__triet_vector_push`→`UnknownShim` P3.5, D bác O 2/2 (bảng 7→8, `mutates_arg` scope-creep), luật 18 dính O ×2. Nợ Nhịp 2 (P-flag canary + checker self-loan-exclusion) chờ G+Giang mở — chi tiết ở nợ SPOF dưới.** **🏁 WO-5 GIẾT UB `free(1)` + KHÉP CHIẾN DỊCH "QUÊN NULLABLE" — O+G ký 2026-07-20, `f432987`+`07ca203`+`61a8136`.** 🔴 **UB SỐNG thứ SÁU (DUY NHẤT hỏng bộ nhớ thật cả phiên):** `Vector<Leaf?>`/`HashMap<_,Leaf?>` (Leaf mang String) → `emit_vector_element_free_loop` bóc `Nullable` TRƯỚC khi free → `emit_heap_free_at` đọc **TAG(=1) làm con trỏ heap → `free(1)` SIGABRT 134**. Vá: refuse container-element heap-nullable ở `Body::verify()` (**Copy-gated** — `Vector<P?>` Copy vẫn chạy); gỡ nhánh chết B2 (RULE7-probe: 0 test chạm). ⚖ **D BÁC LỆNH R2 — ĐÚNG (lần 12/12).** WO của O lệnh refuse local heap `Struct?` "vì policy"; **O tự poison chứng minh refuse local → 15 fixture VỠ** (338-346 `pop`/`remove` trả `T?` = `Nullable(Struct-heap)` local CÙNG MirType với user-viết; `Body::verify` không thấy AST). **WO của O SAI ở R2.** ⚖ **Mâu thuẫn HIẾN PHÁP lộ ra:** §4 "no drop-glue" đấm nhau ADR-0082 `pop`/`remove`. **§15 tu chính:** local/pop-result `Nullable(Struct-heap)` **SOUND** qua `struct_drop` arm (đo Bước ① FREE=1 dup=0); §4 áp HẸP cho repr-slot construction, KHÔNG cấm shape tồn tại. R2 HỦY, fixture 455 = control thường trực. 🦷 **Bài học O: "chạy đúng" + "cùng MirType với tính năng đã ship" ⇒ KHÔNG refuse được ở tầng verifier; kiến trúc sư SAI thì poison-đo rồi RÚT LỆNH, không ép lính.** 🩸 **O sai 6 lần/phiên** cùng gốc "hành động/ra-lệnh trước khi đo" (máy đo mù borrowck · T5 phá B8 · sửa cây khi D cầm · bịa mã lỗi · phân loại local="policy-hole cần bịt" · R2). **D bác 12/12 đúng cả 12**, 5 lần DỪNG-TRƯỚC-KHI-GÕ. Vết D: kỷ luật báo cáo (treo lượt chờ gate 4 lần, 1 lần để poison `panic!` sống trong cây) — O kết luận **giới hạn HẠ TẦNG, cần constraint cứng không phải lời nhắc lần 4**. **🏁 ADR-0065 §14 FULL-SRET NULLABLE AGGREGATE ĐÓNG TRỌN (Lát A + Lát B) — O+G ký 2026-07-20.** **Lát A `Struct?` (`d624cba`)**: tag-prepend `{tag@0,fields@8+}`, slot `total_size+8`. **Lát B `Enum?` unit-only (`c320262`)**: disc-niche, slot `total_size` **KHÔNG +8**, null = `NULL_SENTINEL` ghi thẳng disc@0. **HAI REPR KHÁC NHAU — tách lát là bắt buộc, bê `+8` từ A sang B là hỏng ngay.** Failure-mode gốc CHUNG: local return sret giữ **CON TRỎ**, sentinel-compare so bit-pattern con trỏ với `i64::MIN`, địa chỉ stack không bao giờ bằng ⇒ **nhánh `~0` CHẾT**, trả sai câm; SIGILL 132 chỉ là **thứ cấp** khi rác+rác vượt range ADR-0044. **🔴 HỌ 'match exact, QUÊN Nullable' — NĂM thành viên trong MỘT phiên:** `Enum?` param · `Struct?` param · `Struct?` return · **`Expr::Call` caller-side `is_fat_ret` (D tìm, O sót)** · **`INV-Enum-shape` verifier `triet-mir:1883` (O tự đào bằng grep — `Nullable(Enum)` THOÁT khỏi chính lưới an toàn của verifier)**. ⇒ **§14.7 khắc bất biến: `is_fat_ret` có BA bản sao** (`:305` callee · `:3105` Call caller · `:5219` method-call fail-closed) — ai đụng một bản PHẢI grep hai bản còn lại. **🩸 O SAI 4 LẦN cùng gốc 'hành động trước khi đo'**: máy đo mù borrowck (suýt báo cáo láo — counting harness `lower_source()` BỎ QUA borrowck, đo trên chương trình compiler từ chối) · **T5 ra lệnh phá rào B8 khắc đá trong CHÍNH ADR đang tu chính** (câu cấm gọi đích danh D — D chặn) · sửa cây khi D đang cầm (gate ô nhiễm, trông y hệt hồi quy thật) · bịa hệ mã lỗi `E<code>` không tồn tại ở `LowerError`. **⚖ D bác O 11/11 lần ĐÚNG CẢ 11**, 4 lần **DỪNG TRƯỚC KHI GÕ CODE** để hỏi. 🦷 **Bài học P-E:** buffer thiếu 8 byte **chỉ lộ khi đọc field vượt ranh giới** — T1 một field XANH GIẢ, T3 hai field mới ĐỎ ⇒ răng không vét cạn biến thể là đồ trang trí. 🦷 **Bài học §AMEND:** đọc lại TOÀN BỘ phần KHẮC ĐÁ của ADR gốc trước khi nối — §AMEND kế thừa mọi ràng buộc thân bài, và ràng buộc mạnh nhất thường nằm XA NHẤT khỏi chỗ đang sửa. **🏁 WO-1 `is_empty` TEMP LEAK ĐÓNG (`5a61e74`).** Thành viên CUỐI của họ fast-path-bypass-`emit_shim_call`: `is_empty` owned-String đọc field `len` trực tiếp, sót cách `length` đúng 95 dòng. Đo `FREE=0` rỉ câm, control let-bound=1, anh em `length`=1; **mọi `value` ĐÚNG ⇒ 439 fixture value-based mù tuyệt đối**. Vá 1 dòng `c.push_owned(arg)`; răng `is_empty_temp_leak_counting.rs` assert **`free==1` VÀ `dup==0`** + 2 test chứng minh bộ đếm còn sống. 🦷 **'Cùng code path' KHÔNG suy ra 'cùng trạng thái đã-vá'.** **🏁 WO-1 `is_empty` TEMP LEAK ĐÓNG — O+G ký 2026-07-20, `5a61e74`.** Thành viên CUỐI của họ fast-path-bypass-`emit_shim_call`: `is_empty` owned-String đọc field `len` trực tiếp, KHÔNG qua chokepoint đã vá ở WO-ShimTempOwnership — sót cách `length` đúng 95 dòng. Đo: `is_empty("hello")`/`is_empty(h.name)` **FREE=0 rỉ câm**, control let-bound=1, anh em `length`=1; mọi `value` ĐÚNG ⇒ 439 fixture value-based mù tuyệt đối. Vá 1 dòng `c.push_owned(arg)`. Răng `is_empty_temp_leak_counting.rs` 7 test, 5 shape assert **`free==1` VÀ `dup==0`** (dedup con trỏ — FREE-count đơn thuần mù trước double-free) + 2 test chứng minh BỘ ĐẾM CÒN SỐNG. **🩸 O poison ĐỘC LẬP:** gỡ đúng dòng của `is_empty` → IE-A/IE-B ĐỎ `FREE=0`, LEN-A/LEN-B/IE-A-ctrl XANH (đặc hiệu), md5 khôi phục khớp. Biến thể Vector qua chokepoint = FREE=1 ⇒ KHÔNG cắm test thường trực. **KHÔNG thêm fixture corpus** (giá trị luôn đúng ⇒ fixture là đồ trang trí — G chuẩn thuận). 🦷 **Bài học: "cùng code path" KHÔNG suy ra "cùng trạng thái đã-vá".** **🏁 WO-ShimTempOwnership ĐÓNG — O+G ký 2026-07-19, `04b6174`…`72a0bd6` (8 commit).** Rỉ câm **CẢ MẢNG shim-mượn** (KHÔNG cục bộ ở `length()` như chẩn đoán gốc của O — số đo của D bác): temp vô danh (field-access HOẶC literal) làm argument cho builtin **mượn** không bao giờ được `push_owned` → không ai drop. `concat` 3→1, `contains` 2→0, `eq` 2→0, `length` 1→0 — mỗi ca thất thoát đúng 2 temp; **cả 7 shape in ra GIÁ TRỊ ĐÚNG** ⇒ 439 fixture value-based mù hoàn toàn. Fix tại **chokepoint `emit_shim_call`** tra `arg_consumes` (mượn/thiếu-entry → `push_owned`; tiêu thụ → cấm) + fast-path `length()` vá riêng. **Bán kính rộng hơn phạm vi ký** (quét cả `remove`/`get` key) — G duyệt giữ rộng: *"thu hẹp nghĩa là viết `if name=="remove" { tiếp_tục_rỉ_nhé() }` — đó là NGU XUẨN, cố tình sinh sibling gap"*. **⚠️ Oracle `hashmap_string_key_struct_value_remove_frees_key_and_value` 2→3**: giá trị cũ ghim trên baseline ĐANG RỈ — **O verify pointer-identity `frees=3 distinct=3 dup=0`**, KHÔNG double-free; ai lùi về 2 là tái mở leak. **🩸 O ép poison tới cùng, TIÊU CHÍ CỦA O SAI:** O tuyên "poison ngược không nổ ⇒ reject"; đo hai chiều → M3 bật + bỏ phân biệt = FREE=1 **không nổ** (D đúng), M3 **tắt** + phân biệt đúng = **SIGABRT double-free** ⇒ **M3 (JIT `mir_lower.rs:4717`) mới là lớp chịu lực**, nhánh `!consumed` bị nó che. O rút tiêu chí. **🔴 NỢ SPOF MỚI (phát hiện nhờ mũi poison đặt nhầm chỗ): `builtin_shim_meta().arg_consumes` là ĐIỂM CHẾT** — cả hai tầng (`push_owned` lowerer + M3 zero JIT) đọc CÙNG một bảng ⇒ **KHÔNG phải defense-in-depth**; một entry khai láo thủng cả hai (khai mượn-thực-tiêu-thụ→leak · khai tiêu-thụ-thực-mượn→double-free, **cả hai câm**). ~~Chưa có răng nào canh bảng này.~~ **🏁 P-exist ĐÓNG — ADR-0085 Nhịp 1, O+G ký 2026-07-24, `63887e4`.** Bảng TOÀN PHẦN (8 entry, +`__triet_vector_contains` D bắt · `__triet_string_{append,clear}` `mutates_arg:None` AMEND-2) + cổng tồn tại `Body::verify()` discriminator `__triet_`: `CallDispatch` gọi `__triet_*` mà thiếu meta → `MirError::UnknownShim` ở P3.5 (TRƯỚC borrowck/JIT), machine-fixable, KHÔNG nuốt câm. **🩸 O poison độc lập:** xóa `__triet_vector_push` → fixture 166 nổ `UnknownShim` P3.5 (không silent-miscompile), restore md5 `14a6a39`. T2 (`some_user_fn`→Ok) + 452 fixture xanh chứng minh discriminator KHÔNG giết user-fn. **⚖ D bác O 2 lần đúng cả 2** (bảng 7→8 O sót lower-emit `__triet_vector_contains`; `mutates_arg:Some(0)` O scope-creep tự bắn 5 fixture E2440 self-loan). 🦷 **Luật 18 dính O HAI lần/phiên** (recon nửa vời + đắp cơ chế không hợp calling-convention). **🏁 NHỊP 2a ĐÓNG — self-loan-exclusion + khôi phục `mutates_arg:Some(0)` append/clear, O+G ký 2026-07-24, `7c8cbf2`(fix)+`9a9cb72`(teeth-amend).** M3 mutate-precheck trace `arg`→`real_place` + `.filter(l.dest != *arg)` loại self-loan (mirror tiền lệ U2 `:1260`), `clear(&0 mutable m)`/`append` hết E2440 self-conflict; `mutates_arg:Some(0)` phục hồi → E2440-string-mutate genuine hoạt động. **MIR-grounded (không tưởng tượng):** `_1 = &0 mutable _0; clear(_1)` — args[0]=`_1`=loan.dest, KHÔNG phải `_0` (bác giả định D+O). **🩸 O poison hai lưỡi độc lập:** (a) `.filter(false)` chọc mù precheck → T2 đỏ · (b) over-exclude `source!=real_place` → T2 đỏ (canh false-negative) · (T3) gỡ dòng self-loan → fixture 93 nổ E2440 (teeth false-positive, orthogonal T2). **⚖ O tự bắt T2-clear VACUOUS trước khi ký** (E2440 tới từ borrow-conflict thường tại tạo `&0 mutable`, nổ TRƯỚC precheck; `.filter(false)` vẫn pass) → G Option A: thay T2 plain-local `pop(v)` đập thẳng precheck. 🦷 **Luật 18 dính O BA lần/phiên** (bảng 7→8 · `mutates_arg` scope-creep · T2 vacuous) — quy trình poison cứu cả ba. Sắc lệnh G: mặt trận sau mọi cơ chế phải kèm **map trace MIR**. Vết D: tóm tắt log 3 lần → sắc lệnh reject-thẳng-tay. 🔴 **NỢ NHỊP 2b CÒN TREO (chờ campaign sau):** P-flag behavioral roundtrip canary per consuming shim — oracle FREE-count PHẢI dedup con trỏ (luật 14) để bắt cờ `arg_consumes` khai láo (mượn-thực-tiêu-thụ→leak · tiêu-thụ-thực-mượn→double-free). **🏁 WO-StructReturnRefuse ĐÓNG — O+G ký 2026-07-19, `e7aab8c`.** POLICY GATE refuse `Nullable(Struct)` ở RETURN position — anh em thứ hai của `nullable_enum_return_unsupported` (mirror `Ctx::new`), chặn miscompile câm đã đo ở mặt trận trước (rác câm exit 0 / rác địa chỉ / SIGILL 132 / **SIGABRT 134 mới đo cho struct heap-bearing**). 9 fixture 437-445 (4 refuse + 5 control). Nợ mới ghi sổ: `is_lowerable_nullable_payload` (MIR verifier) cho qua `Nullable(Struct)` VÔ ĐIỀU KIỆN ở return-type position kể cả heap-bearing — đo được SIGABRT 134 thật, KHÔNG suy luận; doc-comment tại đó tự khẳng định sai ("heap fields stay refused via the scalar-only field/payload gate" — số đo bác). Chi tiết ở mục riêng ngay dưới header. **🏁 WO-StructParamABI ĐÓNG — O+G ký 2026-07-19, `7d59b7c`+`ec7ecd8`.** Rác câm ở `Struct?` PARAM (khác cơ chế `Enum?`: pass-by-pointer đọc-xuyên-con-trỏ, KHÔNG copy-in) — fix 1 nhánh `load_place` bare-local trong `mir_lower.rs`, 9 fixture 428-436, nợ mới ghi sổ: `Struct?` RETURN position rác câm+SIGILL không hàng rào (pre-existing, ngoài phạm vi) + `key_marshal` chưa đo. Chi tiết ở mục riêng ngay dưới header. **🏁 WO-NullableEnumParamABI ĐÓNG — O+G ký 2026-07-19, `ccb8db3`.** Chi tiết + nợ lỗ-N1 ở mục riêng ngay dưới header. **🏁 WO-NullableEnumAggregate-Refuse (PA-A) ĐÓNG — O+G ký 2026-07-18, `186bd1c`.** Vá silent-miss SỐNG (`struct Mid{e:E?,m:Integer}` → `mid.m` đọc ra 42 thay 5, exit 0) — chi tiết + phán quyết đảo vai N1/N3 ở mục ADR-0083 dưới. **🏁 ADR-0082 §AMEND-3 GET-BY-VALUE COPY-AGGREGATE KHÓA SỔ — O+G ký 2026-07-15, `28f7c6f`(feat)+`12adc0b`(docs).** `get(container, k)` trả **by-value** aggregate **thuần Copy** (Struct/Enum không heap leaf) từ `Vector<Agg>` + `HashMap<scalar-K, Agg>` — non-destructive (element ở lại, KHÔNG tombstone/free, bitwise-copy). **Heap-bearing aggregate → REFUSE E1049** (mã mới, trỏ `get_ref`); aggregate-key×aggregate-value → defer (refuse cả typecheck E1041 lẫn JIT). Scope + E1049-mới + Vector-HashMap-cùng-slice = G rule 2026-07-14. **Predicate `Type::is_copy_aggregate` mirror `MirType::is_copy` (single-source-of-truth) — KHÔNG `!aggregate_needs_drop`** (over-approx enum-field via `collect_heap_leaves`; **D bác giả định WO gốc của O đúng LUẬT 5, D đúng O sai**, O đính chính §AMEND-3.2). Shim mới `__triet_{vector,hashmap}_get_copy` (`returns_borrow_of:None`) tách khỏi get_ref → borrowck KHÔNG synth loan (§AMEND-3.5); reuse thân get_ref dưới symbol thứ 2. JIT copy-out: thin (stride≤8) ghi thẳng deref'd return, fat (>8) load-loop; shared defensive `!is_copy` guard (Rule#7, latent). **🩸 O verify máu (cp-snapshot restore md5 `a753366b`):** poison `is_copy_aggregate` heap→Copy → `Vector<Tagged{String}>` **double-free 134** (E1049 gate load-bearing, non-vacuous) · **8B-heap-struct T9-masking** (`Wrapper{v:Vector<Integer>}` total_size=8 — leak-câm Slice A) D KHÔNG test → **O probe sống bắt → E1049**, fixture 367 canh vĩnh viễn · positive 361/362/363/366 + counting FREE-count route-lower xanh · E1049 harness assert code-string. **⚠️ D tự bắt+vá SIGSEGV thin-return-deref (363) khai thật; F3 gom guard đối xứng 1 chỗ + bỏ guard hashmap-value trùng. D claim "fmt ran" nhưng edits cuối chưa fmt (LUẬT 2 slip — pre-commit hook bắt, O fmt lại).** Model D=Sonnet 5 (đủ lõi; vết: thiếu fixture masking + fmt-slip, O bù). **Nợ đóng-gói-campaign-riêng:** 🚩 **Deep-Clone heap-bearing aggregate** (campaign LỚN riêng — ADR `.clone()` tường minh + carve-out ADR-0042 move-only + codegen clone đệ quy) · 🚩 **get_ref value-aggregate** (Cụm D/ADR-0081 FROZEN) · 🚩 **drain** (ADR Iteration). ⚰️ ADR-0068 Box/recursive VẪN CẤM CỬA (G nhắc lại). **🏁 ADR-0083 §AMEND-1 SLICE 2 ENUM KEYS KHÓA SỔ — O+G ký 2026-07-13, `91c273a`.** `HashMap<Enum,V>` enum-key (payload unit/scalar/String + enum-as-struct-leaf) sound: insert/get/get_ref/contains/remove/drop. Walker model flat `KeyLeaf` → **recursive emission** `emit_key_hash_value`/`emit_key_eq_value` (disc-mix + `brif`-chain **ACTIVE variant only**, mirror `emit_enum_drop_glue_at` — KHÔNG đọc inactive/padding garbage); key free-loop REUSE `emit_enum_drop_glue_at`; §6 fnptr-first shield unchanged. **🩸 O verify máu (cp-snapshot restore md5 `80fd7ce7`):** DP-E2 reassign-garbage tooth **NON-VACUOUS** (poison tail@+16 → 358 MISS -1; tail rác sau reassign THẬT) · §6-reverse → 354 crash 134 (enum key-class mới rides shield) · struct Slice-1 KHÔNG hồi quy (352/353 green sau walker viết-lại). **⚖ DESCOPE (G rút lệnh "nested-enum MỞ"):** enum variant ôm **aggregate payload (Struct/Enum)** → **REFUSE E1048** — O probe lift-refuse → `HashMap<Shape,_>` roundtrip **MISS -1** chứng minh lowerer fix-8B enum-payload → marshal truncate → silent-MISS. Refuse-over-guess G khen "cứu rỗi bộ nhớ". **⚠️ D bịa 1 chứng phụ** ("enum-in-enum fails MIR verifier in plain match" — O P1 probe bác, plain match chạy=7) → O bắt, D/O sửa doc-comment types.rs (giữ sự thật truncate→MISS, xóa claim MIR-verifier). ~~**🔴 NỢ MỚI G MỞ: "Enum-Payload-Aggregate Sizing Fix"** — lowerer `triet-lower/src/lib.rs` fix-8B enum payload size (fixup pass chỉ struct field, bỏ enum payload) → aggregate payload >8B truncate. Mở khóa nested-enum/enum-struct-payload key khi đóng.~~ **ĐÓNG 2026-07-18 (WO-NullableEnumAggregate-Refuse PA-A).** Sizing đã đúng từ `9a1799c` (ADR-0067 §AMEND co-fixpoint); lỗ CÒN LẠI vừa vá là một bug SIBLING riêng trong `resolve_aggregate_size`'s `Nullable(Enum)` arm (tra nhầm `struct_map` thay vì `enum_map` → luôn MISS → field `E?` payload-bearing giữ seed 8B, tràn sang field kế — silent-miss, không phải sizing-fixpoint-bỏ-sót như chẩn đoán gốc). Refuse tại tầng khai báo (N1, `lower_program`) + fix 1 dòng (N3, `struct_map`→`enum_map`) đã đóng. **⚠️ D probe hậu-vá: N3 alone (KHÔNG N1) đã đủ tự sửa hoàn toàn 3 kịch bản đo được** (struct-field sibling-corruption, nested-field, enum-variant-payload construct+read, cả present lẫn `~0` null arm) — **⚖ G PHÁN QUYẾT 2026-07-18 — ĐẢO VAI N1/N3:** **N3 = SOUNDNESS FIX** (chống tràn size gây corruption — fix THẬT); **N1 = POLICY GATE** (chặn chờ ADR-0065 cấp phép thiết kế repr — **KHÔNG** phải soundness-gate quan sát được qua poison). **O verify ĐỘC LẬP 6 shape** (aggregate payload 24B · đọc ngược chính field nullable · enum-payload-nullable lồng · 3 shape heap — heap bị `heap_type_not_supported` chặn, KHÔNG phải N1): gỡ N1 giữ N3 → **không shape nào corrupt**. G GIỮ N1 tuyệt đối vì bề mặt `Enum?` còn vỡ chỗ khác (nợ SIGILL 132 dưới). **CẤM về sau viện N1 như bằng chứng đã đóng một đường UB.** **🦷 N3 CÓ RĂNG DUY NHẤT** = unit test `resolve_aggregate_size_nullable_enum_reads_enum_map_not_struct_map` (`triet-lower/src/lib.rs` mod tests) — vì N1 chặn MỌI đường fixture-level chạm N3; O verify đỏ độc lập (đảo token → `left: 8 / right: 16`) **+ đặc hiệu** (poison site sinh đôi bare-`Enum` `:570` → test VẪN XANH; site đó do `enum_field_moveout_frees_once_with_cap` canh — cả hai site đều có răng). **⚖ D BÁC WO CỦA O — ĐÚNG (lần 5/5):** O viết teeth protocol "gỡ N1 → thấy 42" **mâu thuẫn với chính số control-biến O đã đo đầu phiên** (`:567` đổi một token → ra 7). **Bài học O khắc: giao thức poison phải đối chiếu ngược với số đo đã có của chính mình — dữ liệu bác WO nằm sẵn trong tay mà không nhảy số.** ~~**🔴 NỢ MỚI (D khui, O verify): `Enum?` PARAMETER ABI VỠ — SIGILL 132.**~~ **ĐÓNG 2026-07-19 (WO-NullableEnumParamABI, `ccb8db3`).** ⚠️ **Chẩn đoán gốc "SIGILL 132" MÔ TẢ THIẾU — failure-mode thật là RÁC CÂM:** `Nullable(Enum)` param không bao giờ được cấp `enum_slots` entry (copy-in `mir_lower.rs` match `MirType::Enum` **exact**, trong khi vòng derived-locals **có** unwrap `Nullable`) → `load_place` bare-local + `GetDiscriminant` rơi fallback `use_var(param_val)` = đọc **bit-pattern của CON TRỎ caller** làm giá trị. Địa chỉ stack không bao giờ bằng `i64::MIN` ⇒ sentinel-compare **luôn phán "present"** ⇒ **nhánh `~0` CHẾT trên mọi biên gọi hàm, trả sai câm, exit 0**. SIGILL 132 chỉ là **ca con**: khi nhánh present bị nhận nhầm đi đọc discriminant thì `SwitchInt` mới rơi default-arm `Trap`. Fix = unwrap `Nullable` tại copy-in (mirror idiom `nullable_payload().unwrap_or`), **chỉ vị trí parameter**; guard `~+` (ADR-0065) + guard return-position không đụng. **9 fixture 419-427 chấm bằng GIÁ TRỊ, không bằng exit code** — oracle-exit-code XANH trên cây hỏng ở ca câm (422). **🩸 O verify máu độc lập (cp-snapshot restore md5 `8dde9fc9`, KHÔNG `git checkout`):** poison-1 gỡ fix → 419/420/421/424/425 SIGILL 132 + **422 ĐỎ bằng giá trị (1≠0)**, 423/426 control xanh · poison-2 chặn nhánh `Enum` **trần** giữ `Nullable` → **426 ĐỎ, 419-425 XANH tuyệt đối** (đặc hiệu) · **O tự đào lỗ verify D bỏ sót: harness là MỘT test đơn `integration_test_corpus()` chạy vòng lặp ⇒ SIGILL ở 419 giết cả tiến trình, 422 KHÔNG BAO GIỜ CHẠY dưới poison-1** → O dựng phép thử riêng đổi `EXPECT: 0`→`777` → `FAIL 422…: expected 777, got 0` chứng minh **T3 có răng ở tầng harness**, không phải file trang trí. **⚖ D BÁC O 2/2 lần, đúng cả hai** (xem mặt trận `Struct?` + nợ lỗ-N1 dưới). **🩸 Bài học O khắc (lần 7): dán nhãn failure-mode SAI** — O ghi "SIGILL 132" cho `Struct?` param, D đo ra rác câm; đo lại 5/5 mỗi bên: đọc **HAI** field (`v.x+v.y`) → SIGILL 132 xác định (rác cộng rác vượt ngưỡng ±(3²⁷−1)/2 → trap range-check **ADR-0044**, hiệu ứng **THỨ CẤP**), đọc **MỘT** field → rác câm exit 0 đổi theo ASLR. **Rác câm là failure-mode GỐC; SIGILL là tiếng sấm đi kèm.** [[feedback_failure_mode_precision]] **🏁 ADR-0083 KEY-AGGREGATE HashMap SLICE 1 (Struct keys) KHÓA SỔ — O+G ký 2026-07-13, `0ebd763`+`1c08a67`.** `HashMap<Struct,V>` struct-key (leaves scalar/String/nested-struct) sound end-to-end: insert/get/get_ref/contains/remove/drop. **ABI G-mandate:** header 24B fixed `[refcount@0][packed@4][hash_fn@8][eq_fn@16]` + fnptr-in-header null-sentinel (Integer/String→NULL, Struct→walker addr) — **§6 dispatch fnptr-TRƯỚC-stride = lá chắn Size-Collision-Trap** (struct 24B trùng String `key_stride` 24; đảo thứ tự → O verify 352 **SIGSEGV 139**, máu G đòi). JIT walker `build_key_hash_walker`/`build_key_eq_walker` đệ quy `collect_key_leaves` (scalar→FNV-mix · String→`__triet_string_hash` · nested→đệ quy; eq short-circuit); key free-loop đệ quy §4; typecheck `is_hashable_leaf` E1048 (Enum-key/collection-leaf/Nullable-leaf REFUSE). Spike `func_addr` fail-fast TRƯỚC walker (G mandate). **🩸 O verify máu độc lập (cp-snapshot restore md5 `0fd4b450`):** §6-reverse→352 SIGSEGV 139 · eq-content-String→ptr-identity→353=-1 (răng thật) · baseline walker JIT THẬT 352=42007/353=42 (MIR dump = `get(struct_key)`, KHÔNG stand-in) · diff BASE-vs-committed = **chỉ doc-comment** (logic byte-identical). **🩸 O bắt 2 Blocker (D vá):** (A) get/get_ref/contains CHƯA wire ở source (E1041, chỉ insert/remove reachable) → D wire overload `exprs.rs:1190` + 2 fixture roundtrip THẬT 352/353; (B) walker correctness chỉ stand-in/compile-only → thêm test runtime; **comment fixture 353 claim hash-poison→RED GIẢ (vacuous do allocator 16-align mask bucket `cap≤16`)** → D sửa thành eq-poison + note trung thực. **Bài học: hash-content KHÔNG tooth được bằng functional-roundtrip cap-nhỏ (alignment mask); cap-LỚN + assert-hash-trực-tiếp mới bắt (ADR-0080 tooth #5 `cap=1_000_003`) — correctness rides on EQ.** **Nợ Slice 2 defer (🚩 cắm cờ):** Enum-key (discriminant + padding-bits + variant-size) · Nullable-leaf key · hash-caching. **⚠️ BOM FIX-2 zero-@8 (Slice B) GIỮ NGUYÊN, chưa đụng.** **🏁 `Vector::pop_front` (O(n) shift, tombstone `len--`) ĐÓNG — ADR-0082 B-α continuation, O+G ký 2026-07-12, `5462c5b`.** Move-out phần tử ĐẦU by-value (`T?`) tái dùng ABI D-1 (`tag@0/fields@+8` marshal + `len--` tombstone); shift xuống O(n) giữ INV-B-α (KHÔNG ring-buffer, KHÔNG hứa O(1)). get-by-value/drain/HashMap-pop_front trục xuất (campaign-nền riêng). 1 commit gộp code+counting-tooth. 7 site (grep-verified 6 ABI-site `__triet_vector_pop` mirror đủ 0 sót); shim B1(rút[0] trước)/B2(`ptr::copy` memmove — overlap len≥3)/B3(`len--` no-zero). **O poison máu độc lập (restore md5 `d90caa4f` mọi vòng):** T-G1 order=132 · T-G2a bỏ`len--`→SIGABRT 134 · T-O1 bỏ pop_front khỏi fat-gate→JIT compile-refuse · **leak counting-tooth thường trực** (`vector_string_pop_front_then_drop_no_double_free` FREE==3; poison→4 RED, pop-back tooth xanh=cô lập) · **T-G2b báo trung thực NON-manifest** (front-pop shift xuống dst<src=memcpy-safe; `ptr::copy` GIỮ vì UB-hygiene, KHÔNG giả đỏ). **Nợ campaign-nền đóng băng (G chốt lôi ra Phase sau):** 🚩 get-by-value (ADR Copy/Clone) · 🚩 drain (ADR Iteration) · 🚩 BOM FIX-2 zero-@8 Slice B. **🏁 CỤM B SLICE C (`HashMap<K,aggregate>` VALUE) KHÓA SỔ — G ký 2026-07-10, `36ba45f`.** insert+drop+alloc value-aggregate (Struct/Enum) SOUND — mirror Slice A/B element push+drop; get/get_ref/contains/remove + key-aggregate REFUSE (get-family chết ở **typecheck** E1041/E1002/E1048, remove refuse ở **JIT** — O probe `.tri` source verify = defense-in-depth có tri thức). 2 commit code: `6ec2630`(F1–F4 fix + T4 unit) · `36ba45f`(teeth). **F1** value-free-loop guard `is_any_heap()`→`aggregate_needs_drop` (mirror Vector 1186); **F2** Enum-arm đệ quy (defense-in-depth latent — frontend chặn enum-payload-aggregate; T4 unit pin); **F3** marshal HAI ĐẦU S3-gap (ĐẦU-A `enum_slots` fat >8 · ĐẦU-B 8B `stack_load` KHÔNG `use_var` — C5/T9 tái sinh); **F4** refuse tách `_key`(key-only)@alloc+insert / giữ `_kv`(K+V)@remove×2+get-family. **🩸 O tự bắt lỗ G bỏ sót ở WO: MÌN-3 ĐẦU-B** (8B-aggregate value ôm 1 handle → `use_var` đọc garbage → LEAK CÂM, 331 fixture không thấy) — tooth **T3** cắm riêng, poison→FREE 0. O verify 4+1 poison→RED độc lập (F1→T1/T2/T3 FREE 0 · F2→T4 false · F3-A→T2 compile-fail · F3-B→T3 FREE 0 · neuter 2 helper→6 refuse "SUCCEEDED"), cp-snapshot restore md5 `62ab04…` khớp, failure-mode = FREE-count-wrong (leak, KHÔNG SIGSEGV). Repurpose `hashmap_struct_value_refused_at_jit`→`..._remove_refused_at_jit` (Luật 3; coverage dương insert-Struct-value chuyển sang T1). **⚠️ Bom hẹn giờ cũ (FIX-2 zero-@8) giữ nguyên, không đụng slice này.** **Nợ mở Slice C (defer):** value move-out (get/remove by-value — nấm mồ chung Vector pop, recursive tombstone) · get_ref borrow value-aggregate (Cụm D/ADR-0081) · contains-allow value-aggregate (follow-up nhỏ) · key-aggregate hash+eq đệ quy. **🏁 CỤM B SLICE B (`Vector<Enum>`) KHÓA SỔ — G ký 2026-07-09, `638b455`.** push+drop sound (heap-payload variants), **pop/by-value move-out REFUSE** (deferred, đòi recursive move-out-tombstone). 8 commit: `c8b8aa6`(S1+S2) · `3bede0c`(S3) · `98a3be2`(AM1 refuse pop) · `a665e96`(AM2) · `a6a41c2`(FIX-1+FIX-2) · `638b455`(11 teeth) + 2 docs. **🩸 O tự bắt: (1) pop fat-aggregate UB pre-existing SLICE A** (verify binary `1e49058`; Slice A teeth chưa từng test pop; deferred-KHÔNG-refuse = UB câm shape P0) → AM1 REFUSE bịt cả A lẫn B. **(2) push+drop UNSOUND — HAI bug che nhau** (BUG-1 `aggregate_needs_drop:1663` thiếu Enum→leak · BUG-2 enum-local không tombstone→double-free; named-case triệt tiêu thành "2 giả sound"). **poison-must-be-red cứu mạng:** first-draft named-tooth đếm nhầm→10/10 xanh giả, poison S2 KHÔNG đỏ mới đào ra. FIX-1(Enum arm đối xứng Struct)+FIX-2(zero payload ptr @8). Teeth làm lại INLINE-anchor non-masking: FIX-1→inline 0 · FIX-2→named 4 · AM1→struct-pop compiles(lỗ A phơi). **⚠️ Bom hẹn giờ ghi sổ:** FIX-2 zero-@8 đúng CHỈ VÌ frontend refuse enum-payload multi-heap-leaf (struct/tuple); nếu gỡ → phải walk mọi leaf. **Nợ mở:** `Vector<aggregate>` pop/get-by-value move-out (recursive tombstone) · scalar-enum disc chưa observe source. **Mặt trận kế = Slice C ĐÃ ĐÓNG 2026-07-10 (xem đầu Mốc).** **🏁 CHIẾN DỊCH READ-SIDE (CỤM A) KHÓA SỔ — G ký 2026-07-04, `37a0723`.** ✅ **A1 get-borrow generic-V**: 6 overload `get` V=Vector/HashMap (Int-key + String-key) → `(&0 V)?`; §AMEND-1 stride-conditional deref giữ invariant `&0 V` bit-for-bit dù local hay get_ref (thin→body_ptr, fat String→cell). ✅ **P0 BÁO ĐỘNG ĐỎ**: pre-existing String-key read SIGSEGV (latent từ ADR-0080 — get/get_ref/contains nhận `&0 HashMap` Reference-wrapped, key_stride chỉ bóc Nullable → default 8 → String-key 24B marshal by-value → hash rác → SIGSEGV) VÁ = unwrap `MirType::Reference` trước match HashMap; cắm cờ 335. ❄️ **A2 get-borrow-mutable** (ADR-0081): FROZEN → Cụm D (functional push/insert ⇒ `&0 mutable V` vacuous khi chưa có deref-assign). 🚫 **V=Nullable**: REFUSE/defer (lowerer chưa match `&0 Nullable`). O verify máu poison→RED độc lập (POISON-1 stride-deref→garbage · POISON-P0 Reference-unwrap→SIGSEGV 139 · overload-break 336/337→E1041); 5 fixture 333-337; restore md5 khớp. **⚠️ Kỷ luật D**: cảnh cáo thép "lần cuối dung túng ném-API-không-test" (D bẻ lệnh giữ String-key overload + thiếu fixture heap-value → G ép bổ sung 336/337). **🔑 ADR-0080 KM-P1a (key-typed `HashMap<String,V>` BACKEND) LANDED — Author+O+G ký 2026-07-03, `c003a5f`. Mechanism sound & sleeping (hand-built MIR + counting): slot `key_stride` 24B fat · `__triet_string_hash` FNV-1a + `hashmap_key_hash/eq` dispatch theo key_stride · key drop-obligation §AMEND-1 out-param ABI (D.1 map-drop-loop / D.2 insert-dup `is_update_out` / D.5 remove-resident `key_out_ptr` — free ở JIT call-site registry-routed → counting-testable, KHÔNG free trong thân shim). O verify 5 teeth poison→RED độc lập (map-drop-leak 1→0 · update-leak 2→1 · remove-leak 1→0 · content-hash alloc-indep cap=1_000_003 · rehash key-stride→NULL_SENTINEL). Chi tiết mục "🔨 ĐANG MỞ" dưới.** **🩹 BUG-E (Outcome-param ABI mis-tag + `~->` early-return heap double-free) ĐÓNG — O+G ký 2026-07-03, 2 WO liên tiếp (`ddb7841` param-ABI copy-in gap + `818602c` early-return heap-payload double-free, 3 site). Chi tiết đầy đủ ở mục "✅ ĐÓNG — Bug-E" bên dưới.** **🩸 GET-BORROW HEAP VALUE (ADR-0079) IMPLEMENTED/CLOSED — G ký 2026-07-01.** Read-side container khép: `get(&0 container,k)→(&0 V)?` zero-copy borrow (P1 V=String). Borrowck whole-container loan (U2 PropagatedLoan builtin + U3 mutate-while-borrowed E2440 cho consume insert/push + in-place remove/pop); JIT shim trả con-trỏ-slot (0 alloc), not-found→NULL_SENTINEL. O verify máu: content-read `length`→2/5 · source-level E2440 · 5 borrowck teeth poison. Slice A `a970540`. **🏁 TYPED HEAP-CONTAINER P1 ĐÓNG TRỌN — ADR-0077 (Vector) + ADR-0078 (HashMap) SEALED, G ký 2026-07-01.** `Vector<T>` + `HashMap<Integer,V>` (T/V = built-in heap: String/Vector/HashMap/Nullable) chạy sound end-to-end source qua JIT real-allocator: construct + push/insert(Move) + pop/remove(move-out `T?`/`V?`) + drop — không rỉ một byte. Element/value-SIZE = hằng compile-time (tách-tầng khỏi native-layout). **KEY-typed (`HashMap<String,V>`) + UserStruct value + get-clone/borrow heap value = defer Tầng 2/P2.** HM-P1b 3 vòng O-reject ép chân lý: garbage `lower_type` bỏ value-arg → vacuous-tooth (literal-temp no-drop-obligation) → named-local poison `arg_consumes[2]=false`→SIGABRT 134 ĐỎ. **🏁 KỶ NGUYÊN NULLABLE KHÉP HOÀN TOÀN — ADR-0076 SEALED (heap-`T?` trong aggregate field/payload, giao điểm B8 cuối).** Lát đơn atomic `6327890`: 5 mũi (gate-lift + field-layout sentinel + drop-arm `collect_heap_leaves` + construct/widen + borrowck). Cổ tức PA-3c: conditional-drop = sentinel-no-op, KHÔNG `brif`. O vồ double-free CASE B (match-present-bind-move → SIGABRT 134, borrowck im) → D đóng STATIC tag-niche-tombstone (KHÔNG dynamic-flag). O verify máu 3 tooth (Deinit-after-bind 134 · sinh-tử `is_copy(Nullable(heap))==false` 7-leak · drop-arm). `let s=b.s`→E2423 (Nợ defer giữ). **🔒🏁 CAPABILITY Ł3 (ADR-0069) NIÊM PHONG — COHERENCE VISION §8 HOÀN TẤT.** Đại số Ł3 khép kín ba chân: null(PA-3c) / logic(Trilean) / **capability**. ZST-token ngậm Ł3-Trit: Grant(+)/Ambient(0)/Deny(−) tĩnh zero-cost + Defer(Unknown) runtime trap `user(2)` fail-closed. Lát 0 `8b06a28` (ZST & cấm copy, 2-classifier defense-in-depth) · §amend-A `47eb283` (M1 receive-only) · Lát 2 `ca8272e` (possession E2212) · §5 `d84cd24` (mint-site lock) · Lát 3 `2dd4d5f` (Defer hook — O verify 4 răng, R-fail-closed boundary `≤` là tử huyệt) · Lát 4 demo fixture `278` (end-to-end →30). Mã mới: E2211 (mint non-grant) · E2212 (deny possession). **Mặt trận mandate ternary-first (G+Giang 2026-06-22) ĐÓNG.** **🏁 Heap-aggregate cluster ĐÓNG TRỌN** — ADR-0070 partial-move + ADR-0071 import `::` + WO-0073/74/75 (heap-nullable-return drop-glue · enum-field move-out · multi-level `h.inner.x` projection-path), origin/main = `0947482`.

### 🎯 MẶT TRẬN KẾ — **G CHỐT 2026-07-28, ƯU TIÊN P0**: `WO-SRet-Aggregate-StringField-Corruption`

Vá triệt để lỗi **sret trả struct chứa field `String`/heap ra giá trị RÁC** (chi tiết
đo đạc ở mục "NỢ LEO THANG" ngay dưới). **P0 tối cao — G cấm mở BẤT KỲ chiến dịch
tính năng hay cú pháp nào trước khi bóp chết lỗi này.**

🔴 **Vì sao xếp trên cả ba họ crash vừa vá:** nó **SAI CÂM, `exit 0`**. `134`/`139`/`132`
bị OS cắn cổ ngay và test harness thấy liền; con này trả `94060113734544` rồi thoát
sạch — **mọi cổng CI bật đèn xanh cho dữ liệu rác**. Đã đo tồn tại sẵn trên `35f4f02`
(worktree sạch dựng riêng), tái hiện với **0 tham số**.

Thứ tự bắt buộc (giữ nguyên khuôn đã thắng 2 chiến dịch liền): **recon-trước →
trình bản đồ → G duyệt → soạn WO → D thi hành → O verify máu**. ⚠️ Cơ chế nghi can
(`_0` sret không có `struct_slots` entry ⇒ bước sync `len@+8`/`cap@+16` tại
`mir_lower.rs:~3156-3168` không chạy) mới là **chẩn đoán của D, O đã xác nhận triệu
chứng + tính pre-existing nhưng CHƯA đo cơ chế độc lập** — phiên sau PHẢI tự dump
MIR/JIT, không chép khung.

---

### 🏁 ĐÓNG — **`WO-Param-Aggregate-CopyIn`** (O verify máu + G ký, ADR-0066 §AMEND-1, `0f11ede` + tooth bổ sung 555)

O verify bằng chính poison copy-in: sites #36-#39 (`vector_push`/`hashmap_insert`
fat/8B element) chuyển từ **refuse fail-closed exit 4** (trước vá) sang **chạy
đúng** (sau vá) mà 0 fixture nào canh — đóng bằng fixture 555
(`555_param_aggregate_copyin_container_push_pop_tooth.tri`, Struct param heap-bearing
push trực tiếp vào `Vector<Leaf>` rồi pop+match, oracle giá trị thật `length(popped.s)==2`,
KHÔNG oracle exit-0). Poison → exit 4 đúng message; khôi phục md5 khớp.

D đo lại cơ chế bằng MIR/JIT dump (không chép khung recon cũ): callee prologue
cấp `StackSlot` + copy-in `layout.total_size` byte cho MỌI param `MirType::Struct`
thuần (mirror String `mir_lower.rs:~2691` / Enum `~2701` / Outcome `~2748` — Struct
là aggregate ABI thứ TƯ còn thiếu bước này). `Nullable(Struct)` param GIỮ NGUYÊN
refuse fail-closed (ngoài phạm vi). Chi tiết đầy đủ (bảng 49-site, 8 fixture
543-550 + 4 control 551-554, counting `param_aggregate_copyin_counting.rs`,
canary `Deinit(arg)`, poison thủ công cp-snapshot cả `mir_lower.rs` lẫn
`triet-lower/src/lib.rs`) ở `docs/decisions/0066-heap-in-aggregate-move-drop-glue.md`
§AMEND-1.

🔴 **NỢ LEO THANG phát hiện lúc verify (KHÔNG sửa, ngoài phạm vi WO này):**
trả **struct-theo-giá-trị (sret, non-nullable, KHÔNG phải `Struct?`)** chứa một
field `String` sinh **giá trị RÁC (không crash)** — `_0` (sret) vĩnh viễn không
có `struct_slots` entry nên bước đồng bộ `len@+8`/`cap@+16` cho String
(`mir_lower.rs:~3156-3168`) không bao giờ chạy khi field đích là sret qua một
Assign chiếu-field. **TÁI HIỆN VỚI 0 THAM SỐ** (`function make() -> Leaf {
let p = Leaf{s:"hi"}; return p; }` đã lỗi) — độc lập hoàn toàn với param, lỗ
tồn tại từ trước (440 chỉ khoá case `Struct?`/Nullable, chưa từng có fixture
chạm case PLAIN). Chưa có WO — cần recon riêng trước khi mở.

🟡 **Nợ nhỏ O đo (ghi sổ, KHÔNG sửa code):**
1. `step_by(8)` trong copy-in ghi lố slot khi `total_size` không phải bội của 8
   (`triet-mir/src/lib.rs:~1590`, xảy ra khi mọi field là Trit/Trilean/Tryte).
   O ép 2 probe → **hiện KHÔNG quan sát được** (Cranelift đệm frame che dấu).
   Mẫu có sẵn ở nhánh Enum copy-in (`mir_lower.rs:~2743`), không phải nợ mới
   do WO này đẻ ra — chỉ kế thừa. Răng-khi-có-đường-chạm (chưa cắm).
2. `param_aggregate_copyin_counting.rs` chạy **in-process**, không
   subprocess-isolated — dưới poison (copy-in tắt) cả binary test chết
   (SIGSEGV/SIGABRT thật, đã đo ở round 1 khi poison nhầm site Deinit), 6
   assertion không báo riêng lẻ từng cái. Răng THẬT (đã đo tự abort dưới
   poison) nhưng thô ở chiều đỏ — cải tiến sau nếu cần cô lập từng ca.

### ✅ KHÔNG CÓ WO ĐANG MỞ (2026-07-26(b)) — phiên đóng sạch tại `780e081`

> ⚠️ ANTI-ZOMBIE: mục "🔨 ĐANG MỞ WO-Front-A/ADR-0086" cũ (ngay dưới) ĐÃ ĐÓNG
> từ `dbde2d5` (2026-07-24) — O verify + G ký xong nhiều phiên trước. Giữ nguyên
> văn cho lịch sử; KHÔNG phải việc đang mở. Backlog THẬT = §nợ campaign-riêng ở
...
`{message, span}` KHÔNG mã lỗi → `enum` 8 biến thể (`ConstructNotYetLowered`
E1100 · `NullableEnumPayloadUnsupported` E1120 · `NullableStructReturnHeapField`
E1121 · `EscapingClosureSealed` E1122 · `UndefinedLocal` E1140 ·
`NullLiteralWithoutExpectedType` E1141 · `LiteralOutOfRange` E1142 ·
`InternalInvariant` E1190), `#[derive(thiserror::Error, miette::Diagnostic)]`
mirror `CapabilityError`. Taxonomy + 2 ruling biên (`:5419`→E1100 trait-method
deferred-ABI, `:5935`→E1122 Lambda YAGNI seal) ghi ở ADR-0086. 8 named
constructor giữ nguyên chữ ký (47/47 call site không sửa); 39 điểm dựng nội
tuyến đổi `LowerError { .. }` → `LowerError::Variant { .. }`. Message text
giữ NGUYÊN VĂN mọi nơi (kể cả `[Fix]` blocks) — fixture `// ERROR: <substring>`
corpus không cần sửa (khớp qua `Display`, không qua mã). `Cargo.toml` thêm
`thiserror`+`miette` (workspace version). Driver (`main.rs:67-75`) đổi
`eprintln!("lowerer error: {e}")` trần → `miette::Report` + `NamedSource`,
mirror cách driver render lỗi parse/typecheck. Test mới
`crates/triet-lower/tests/diagnostics.rs` (8 test, mirror
`triet-typecheck/tests/diagnostics_format.rs`) khóa cứng `.code()` cho cả 8
mã; `E1120`/`E1121` trigger qua fixture thật (414, 440) chạy full
parse→typecheck→lower, không hand-built — chứng minh mã phát ra từ pipeline
thật. Unit test `lambda_is_sealed_yagni` (lib.rs) đổi từ field-access
`err.message` (vỡ compile khi struct→enum) sang `matches!(err,
LowerError::EscapingClosureSealed{..})` + `err.message()` accessor.
...
### 🔴 NỢ MỚI (2026-07-20, phát sinh từ WO-2)

1. **`LowerError` KHÔNG có hệ mã lỗi — trái `CLAUDE.md`.** Grep xác nhận: `#[derive(Debug, Clone)]` trần,
   **không `miette::Diagnostic`, không `#[diagnostic(code(...))]`**. `CLAUDE.md` quy định *"All errors implement
   miette::Diagnostic; every error variant gets `#[diagnostic(code(triet::<area>::E<code>))]"*. ⇒ mọi refuse của
   lowerer là prose không mã, **không machine-fixable theo ADR-0027**. Pre-existing, G cho ghi sổ chờ campaign dọn rác riêng.
   *(O từng ra lệnh cho D dùng header `E<code>`; D verify, phát hiện hệ đó không tồn tại, lệch có báo cáo — không bịa mã giả.)*

2. ~~**`mir_lower.rs:3730` PANIC thay vì `Err` khi caller/callee lệch `is_fat_ret`** (index `inst_results` không kiểm trước).~~
   **🏁 ĐÓNG 2026-07-25 — WO-Robust-InstResults, O verify máu + G ký, `fc51f05`+`b270df4`+`3fc170f`.** Toàn bộ **10 site
   `inst_results(call)[i]` production** → helper checked `nth_call_result(...)?` → `JitError::CallResultArityMismatch
   { callee, expected, got }` (struct variant, Display "please report" mirror `Internal`; ICE KHÔNG đổ lỗi user).
   Track B rule #1 thỏa. 🦷 Teeth hand-built MIR: shim `has_return:false` + CallDispatch `dest` non-empty +
   `ReturnShape::Scalar` (ép đọc `[0]`) → `Err(CallResultArityMismatch)` **tiến trình sống**; **🩸 O tự re-poison site
   4764** (`nth_call_result?`→`inst_results[0]` trần) → PANIC `index out of bounds len 0 index 0 @4764:43`, restore
   md5 `da3a0d80` khớp. Class ICE này khai tử. D=Sonnet 5 (agent mới sau spend-limit crash bản cũ), 0 bế tắc, khai
   thật 4 lệch nhỏ. §14.7 + INV-Enum-shape vẫn là *phòng* (giữ nguyên); helper này là *đỡ* runtime.

3. **`Struct?` / `Enum?` return qua cú pháp method-call = OVER-REFUSE.** Bản sao #3 (`triet-lower/src/lib.rs:5219`)
   fail-closed (`Err(LowerError)`), **KHÔNG phải UB** — chứng minh bằng probe (fixture 448 Lát A, 453 Lát B),
   không bằng đọc code. Chờ lát riêng.
...
- Defer: get-borrow-mutable (A2 FROZEN, Cụm D) · ~~V=Nullable get_ref~~ **🏁 REFUSE ĐÓNG 2026-07-25 — WO-GetRefNullableRefuse (E1051, ADR-0088 defer), `6ebef58`+`768c631`+`62087a7`+`36dad8e`.** Recon lộ design cliff double-nullable (`get`→`V?`; V=`T?` ⇒ `(&0 T?)?`/`T??`, outer=key-found vs inner=stored-null XUNG ĐỘT; silent-flatten=wrong-result). G chọn refuse sạch: guard `exprs.rs` sau heap-violation → E1051 (chỉ `name=="get"`, KHÔNG `contains`), `return Type::Unknown` dập cascade. **🩸 O poison lộ tầng dưới CŨNG vỡ** (`MIR verification: heap-nullable T? not yet lowered` khi gỡ guard) — double bằng chứng refuse-tại-typecheck đúng chỗ. Fixtures 465/466/467 refuse + 468 control. Nợ backlog: **ADR-0088** (double-nullable campaign type-system — định nghĩa `T??` mọi tầng AST/typecheck/MIR/JIT/match; ghi tension C6 auto-flatten `mir_lower.rs:4030` CHỈ ở `?+>` operator, không toàn cục). · V=UserStruct (P2 native-layout).
