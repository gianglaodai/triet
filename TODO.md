...
### 🔴 SỔ ĐỎ — **Import path `.` → `::`** (G phán 2026-06-25, án tử hình dấu `.`)
Giang khai thật: chọn `.` cho import theo quán tính Python/Java thay vì học `::` của Rust; G chỉ ra `::` ưu việt cho **sự trong sáng AST** (tách bạch path-separator khỏi field-access `.`). **Trả `::` cho đường dẫn import.** ⚠️ **ĐẢO ADR-0005** (dot-paths đang LOCKED: `crate.foo.bar`, `from std.io import`) → **cần ADR MỚI supersede** (KHÔNG revisionism câm — sửa ADR-0005 status→superseded). Sweep RỘNG: lexer token · parser path · MỌI `examples/*.tri` + fixtures · docs (SPEC/CLAUDE.md bảng convention §Language) · error messages. G **đích thân giám sát** khi mở. **Thứ tự: SAU Partial-move** (G phán "ngay sau Capacity" = mặt trận kế, xếp cạnh).

### 🔴 SỔ ĐỎ — campaign độc lập KẾ: **Partial-move & Struct-ZST** (G khắc 2026-06-25)
`let v = hw.vga` (field-level move-state) = **con quái vật lõi Borrow-Checker / Memory-Management**, KHÔNG phải capability — G cấm nhồi vào ADR-0069 (scope creep, "mổ tim xong đừng mổ nốt dây chằng"). Mặt trận RIÊNG: ADR mới + poison tests (rã struct · move một nửa · xài lại nửa kia) + dọn **B8 gate `triet-lower/src/lib.rs:72`** (lầm ZST-capability-field với heap → reject `struct Hardware{vga:VgaBuffer}`). Mở khóa luôn schema §10 `kernel_main(hw)` destructure-move = canonical proof Hardware-Token (Lát 4-full đã defer sang đây).
