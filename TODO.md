...
- [ ] **jit.4.agg.cross-call (DEFERRED — now the dominant blocker, `call`≈1461)** — cross-mode call marshaling (boxed caller → unboxed callee + vice-versa: unbox args / box result at the boundary). Currently both directions tier down (the agg.1c-guard + 1c-iii guards). Boundary-risk slice — design carefully, may need an ADR note. This is the single biggest remaining coverage lever after agg.3b.
