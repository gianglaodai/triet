...
- [ ] **v0.9.x.atomic.5a** — Loader `sys` synthetic root: extend `loader.rs::load_stdlib` to declare `sys` root containing `module atomic`/`module raw_thread`. Mirror `std` pattern. Per ADR-0029 §2 mandatory lockstep, also port `compiler/modules.tri` loader if it has stdlib root declaration. Verify `capability_gate_e2e` 20 tests still pass.
- [ ] **v0.9.x.atomic.5b** — Stdlib `std/sys/atomic.tri` file: `enum Ordering { Relaxed, Synchronized, Strict }` per ADR-0028 §3 + function signatures cho 10 builtins. Update atomic_counter demo to pass Ordering arg.
- [ ] **v0.9.x.atomic.5c** — Stdlib `std/sys/raw_thread.tri` placeholder (atomic_counter imports `spawn` — needs declaration để self-consistent). Close check_resolved.rs cross-module Atomic typecheck path (deferred từ .1).
