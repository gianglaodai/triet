...
- [~] **jit.4.agg.cross-call** — cross-mode call marshaling.
  - **cross-call.a (boxed→unboxed, SCALAR-only)**: `boundary_class` + `__triet_unbox_scalar` shim + `emit_box_scalar`/`emit_unbox_scalar` + `func_sigs` in `ProgramContext`. A boxed caller calling an unboxed callee unboxes scalar args (ptr→raw) + re-boxes the scalar result; `Unit`/`Long`/composite boundaries tier down. **JIT-able 1298→1318 (32.8%→33.3%, +20).** Audit now splits `cross-mode` (1257) from same-mode `call` (172).
  - **clone-on-return `b90dfed`**: closed the same-mode **boxed** latent double-free (a boxed fn returning a borrowed param handed the borrowed `Rc` ptr to the caller as owned → caller + owner both drop). `__triet_clone_arc(ptr)->ptr` (+1 strong count) shim + `emit_clone_arc`; the boxed Ret clones iff the returned `Value(id)` ∉ created_boxed (created box / inline-Const = already owned). Regression test `keep(p,q)={ _=p.field(0); ret q }`. Coverage unchanged (1318) — pure correctness fix.
  - **KEY FINDING — refcount discipline is CROSS-CUTTING (ADR-level decision, author's call):**
    1. The **unboxed** Ret arm (`codegen.rs:1931`) ALSO lacks clone-on-return → the SAME latent double-free exists for an unboxed fn returning a borrowed *composite* param (`id(s:String)=s`), present since jit.2b. Not yet triggered (composite-in-unboxed is narrow). Fixing needs the `TypeTag` (composite return → clone; Integer i64 → no clone — Cranelift type alone can't tell `String` ptr from `Integer`), touching the perf-sensitive fast path.
    2. **Cross-mode composite** (the big lever, most of the 1257) needs clone-on-return on BOTH sides: the unboxed callee won't bump refcounts, so the boxed caller must clone the composite result after the call (over-clones → bounded dev-tier leak when the callee returned a fresh box — leak-vs-correctness tradeoff). AST nodes = structs = `TypeTag::Unit` (ambiguous), so even with composite marshaling, `Unit`-typed boundaries stay tiered down until the lowerer distinguishes struct/enum from true-Unit (a `TypeTag` change — bigger).
    - **Recommendation:** write an ADR for the unified clone-on-return refcount discipline (both modes + cross-mode) + the leak tolerance, BEFORE more cross-mode work. This is the "design carefully, may need ADR note" the author flagged.
  - [ ] **cross-call.b (unboxed→boxed, scalar)**: symmetric in `translate_call` (box scalar args → temp boxes dropped after call, unbox scalar result). Smaller lever; do after the clone-on-return ADR.
