...
- [x] **jit.4.agg.cross-call** — cross-mode call marshaling + the refcount discipline ([ADR-0035](docs/decisions/0035-jit-boxed-refcount-discipline.md), Locked).
  ...
  - [ ] **cross-call.b (unboxed→boxed scalar)**: symmetric in `translate_call`. Smaller lever; remaining same-mode `call` (192) is mostly Unit boundaries (see §4 ceiling below), so do only if it shows coverage.
