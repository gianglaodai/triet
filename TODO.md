...
- [ ] **v0.9.x.atomic.7** — Demo runtime exercise. Phương án A locked 2026-05-30 (author "chậm mà chắc"): ship borrow expression syntax + E2420 enforcement minimum; NLL/lifetime/multi-thread defer v0.10 per ADR-0031 §10 backlog. Split into 5 sub-tasks:
  ...
  - [ ] **v0.9.x.atomic.7d** — **E2420 UseAfterMove real fires** per ADR-0031 §4 + ADR-0025 §5. Typecheck-level move-tracking: CFG walk over function body, mark binding state alive/moved on move site (`&+ x` consume + move-into-`&+ T` parameter), fire E2420 on use of moved binding. Tests: positive (double-`&+` fires, use-after-`&+` fires), negative (single-use clean, `&0` borrow no-consume clean), mixed cases.
