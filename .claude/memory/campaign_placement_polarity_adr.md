...
# ROUND 4 (2026-08-25, later) — Author ruled, G signed. L27–L30.

> Context: the Author reframed the whole campaign — **do not judge this design against existing ADRs;
> judge whether it is good, serves Triết's tenets, and is implementable.** He is willing to bin old
> ADRs. O's verdict, unchanged after re-testing from scratch: the design is the **strongest ternary
> claim the project has produced**, because G's agency axis **predicts 3/3** transitions and the type
> system derives the same three independently — a decorative trit cannot predict. **But it needs no
> teardown**: `&+` appears in 9 `.tri` files (5 are refuse fixtures), `&-` in **0 fixtures**, bare
> parameters were already owned, and `+T`/`-T` are purely additive. **What goes in the bin is PAPER**
> (ADR-0022 §2/§5.1/§6.3, SPEC §10.1/§10.3–§10.5, four phantom ADR numbers) — a supersession table in
> one new ADR, not a rebuild. The compiler is what is expensive; the last teardown cost a working
> backend plus a 1637-test net and the project is still climbing back (581 fixtures).

| # | Ruling |
|---|---|
| **L27** | **`T → -T` is REFUSED** (Author ruled; G signed). `immortal` on a `+T` is **0 instructions** — a type-level relabel that merely omits drop glue. On a `T` it is `__triet_string_alloc`/`__triet_vector_alloc` (`crates/triet-jit/src/mir_lower.rs:5686`, `:6083`) + an O(sizeof(T)) copy + a recursive move of every heap leaf out of the frame. **One keyword, two behaviours, {0 bytes} vs {alloc + O(n) copy}** ⇒ Pillar ①. A **warning is refused** by the **EMPTY MIDDLE** (the argument that already killed `lazy`+`init` at `:88` and the mandatory-message spelling at **L14**): if the operation is fine the warning is noise everyone suppresses, and a construct whose only recommended use is suppressing its own diagnostic is dead syntax; if it is not fine the warning is a refusal in costume. Prior art agrees — Rust has no `leak(x)` on a stack value, only `Box::leak(Box::new(x))`, **two words with the allocation named separately** — and `Box::leak` is the very precedent L11 invokes for C2. Diagnostic: *"cannot promote a stack value to immortal — `immortal` surrenders the obligation to free, and a stack `T` has no such obligation to surrender"*, fix-it naming **two** lines. |
| **L28** | **`let x: +T = <expr>` is LEGAL and is the ONLY spelling of allocation.** ⚠️ **COUPLED TO L27 — not independently overridable:** allowing `T → -T`, or forbidding `let x: +T = <call>`, are the two ways to turn L27's fix-it into a lie. **L4's replacement sentence, verbatim into the ADR (B8):** *"Coercion happens against a DECLARED target type, never an inferred one. It may only ERASE placement into a borrow. It may NEVER INTRODUCE placement. Introducing placement is **not a coercion at all — it is an ALLOCATION**, and it has exactly **one spelling: the initialiser of a `let` binding whose declared type carries `+`**. In every other position — argument, return, tail expression, struct-literal field, container element — supplying a `T` where a `+T` is expected is an **error** whose fix-it names the `let`. Borrowing is implicit; allocating never is, and allocation has exactly one spelling."* **Semantics: allocate, MOVE the bytes in, tombstone the source** (ADR-0042) — not a copy (two owners of one buffer = double free), not a deep copy (hidden O(n)). This also hands the user the `T`-to-heap path for free: `let boxed: +Config = my_local`, source dead after (E2420). 🩸 **O's own wording ("a `let` with a declared target type IS an allocation") was TOO LOOSE and must not ship** — applied literally it also fires at argument / return / field-init / container-element positions, **4 of 5 of which put the allocation on a line bearing no `+`** — the very hidden allocation L27 refuses, entering through the front door, and handing placement back to the library against **L17②**. **O under-enumerated — the second time this campaign** (`:169`). **L7 must be restated in terms of the BINDING, not the struct literal** (B8). |
| **L29** | 🔴 **NEW REFUSAL — `-T` (or an `-F` projection under L19) is NEVER a legal operand of a `+`-introducing `let`.** `let x: +Cache = app.cache` where `app: -App`: L19 types the projection `-Cache`, and `-T → +T` is refused (agency cannot be reclaimed). L28 now *looks* universal, so a D will apply it here and **reclaim an immortal object into a freeable one = a double-free generator.** This is the hole L28's answer opens. Needs a tooth. **Derived dependency: the static-suppression property of L30 rests on this refusal**, exactly as L24 rests on L13 — record it, give it a tooth. |
| **L30** | **L11 CORRECTED — C2 needs neither "nothing" nor the header sentinel.** ✅ **Measurement confirmed** (and O missed a third shim): `__triet_string_free` (`mir_lower.rs:5731-5741`), `__triet_vector_free` (`:6125-6138`) and **`__triet_hashmap_free` (`:6784-6795`**, a 24-byte header, `HASHMAP_HEADER_SIZE:6511`) all guard only `ptr == 0 \|\| ptr == NULL_SENTINEL` then `dealloc` — **none reads the header**; `ObjectHeader` (`triet-core/src/memory.rs:51`) is constructed nowhere outside its own module (the JIT writes the 8 bytes by hand at `:5699`, `:6096`, `:6746`); `new_frozen_forever` and the `increment`/`decrement` sentinel skips have **0 callers**. ❌ **But O's INFERENCE is REFUTED** — it conflated *which bytes* with *which drop site*. Under unique ownership (L1, no `Rc`), after `immortal raw` (**mandate that it MOVES**) the sole owner is a local whose static `MirType` bears `-`, so **the drop-site set to suppress is fully static whichever branch ran.** Tooth #5 (`-App{cache:+Cache}`): the local is `-App`, `emit_heap_free_at` (`:1769`) is never entered, FREE == 0 statically. Tooth #4 (`+Session{config:-Config}`): `emit_struct_drop_glue_at` (`:1780-1783`) walks fields by **declared** `MirType`, skips `-Config`, FREE == 1 statically. **And both are measurable TODAY** — `crates/triet-driver/tests/` already holds **46** `*_counting.rs` harnesses. **The sentinel path is REFUSED, not deferred:** it taxes every mortal program with a load+branch per free to serve immortal ones (Pillar ② inverted); root-only stamping is unsound under it (interior objects still read `refcount=1`) so it would need a recursive runtime stamping walk roughly the size of drop glue itself, to replace a guarantee already held at compile time; and the refcount half exists for ADR-0026 §3.2's `Send` boundary, which is **frozen** by hard rule #9. **C2's real cost: ① the polarity-carrying `MirType` variant (L9 — `+T` pays for it anyway) · ② one arm in the drop-glue dispatch at `mir_lower.rs:1769-1814` and one in `emit_struct_drop_glue_at` (two arms ⇒ **two orthogonal poison spears**, G-Law 29) · ③ `immortal` moves its operand.** Nothing in `triet-core/src/memory.rs`, nothing in the three shims. |

## 🔴 B6 WIDENED + B7/B8 added — G's sign-off stands at BLOCKED

- **B6 is aimed at too few predicates.** `MirType::is_any_heap` (`crates/triet-mir/src/lib.rs:650-652`) is a **closed positive `matches!`** over `String | Vector | HashMap`, with **54 call sites** (`triet-jit/src/mir_lower.rs` 20 · `triet-lower/src/lib.rs` 18 · `triet-mir/src/lib.rs` 10 · `triet-borrowck/src/checker.rs` 6). For `-T` the default is correct **by accident** (false ⇒ no glue ⇒ what we want). For `+T` it is **silently wrong**: a new `MirType::Placed(Plus, String)` returns **false** ⇒ no free emitted ⇒ **leak with exit 0 at up to 54 sites.** Widen B6 to cover `is_any_heap` and its call sites, with the **fail-CLOSED** default written down as a documented invariant and **one poison per direction** (G-Law 30).
- **B7** — ONE refusal table in the ADR: `{T, +T, -T}` × `{let, argument, return/tail, struct-field init, container element}`, including the **L29** cell. Build the table before a D types a line (G-Law 22).
- **B8** — L4's amended sentence and L7's binding-restatement go into the ADR **verbatim**, not paraphrased.

## LESSONS (round 4)

- 🩸 **O over-conceded for the SECOND time in this campaign, the same shape both times.** He took a real measurement (dead sentinel machinery, 0 callers) and **promoted it past its warrant** into a design requirement it could not support. The conclusion would have bought a recursive runtime stamping walk plus a per-free branch tax **to replace a compile-time guarantee already held**, and would have dragged `-T` onto frozen concurrency machinery. **The pattern to name: O's strongest-FEELING argument keeps measuring weakest** (round 3's separate-compilation argument, `:170`, was the first).
- 🩸 **O under-enumerated for the second time** (`:169`): the L28 boundary wording covered the `let` he was thinking of and silently admitted four positions he was not.
- 🩸 **O stated "two free shims" where there are three** — `__triet_hashmap_free` has its own 24-byte header layout. G-Law 19: your own grep pattern is also an assumption.
- ⚖️ **G overruled O's Pillar-② framing as the weakest of the three available arguments** while agreeing with the conclusion — the ruling survived, the reasoning was replaced. Demand the strongest argument, not the first sufficient one.
