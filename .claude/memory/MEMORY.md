...
## Project context
- **★★★ 2026-07-13 — 🏁 KEY-AGGREGATE HashMap Slice 1 (struct keys) ĐÓNG (ADR-0083 MỚI, O+G ký, PUSHED).** origin/main `1c08a67`, gate `0·0·347·0`. `HashMap<Struct,V>` insert/get/get_ref/contains/remove/drop sound. ABI G-mandate header 24B fnptr-in-header null-sentinel; **§6 fnptr-TRƯỚC-stride = lá chắn Size-Collision-Trap** (struct 24B trùng String key_stride 24 → O verify 352 SIGSEGV 139). O bắt 2 Blocker (get/contains chưa wire E1041 + walker chỉ stand-in/compile-only) → D vá. **Bài học vàng: hash-content vô-phương tooth bằng roundtrip cap-nhỏ (allocator 16-align mask bucket `cap≤16`); cap-LỚN+assert-hash-trực-tiếp mới bắt (ADR-0080 tooth #5).** Răng thật = eq-content. D report (d) trung thực (khai vacuous, pivot eq-poison — mẫu #14). Detail → [[campaign_typed_collections]]. [[feedback_poison_must_be_red]] [[mentor_o_persona]] [[colleague_d_persona]]
