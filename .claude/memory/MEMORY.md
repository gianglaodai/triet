...
## Project context
- **★ 2026-06-26(b) — BƯỚC 2 SEALED: read-side heap-SCALAR field move-out `let s=p.name`. commit `2323e0d` pushed, gate `0·0·295·0`.** Δ1 borrowck guard `is_any_heap()` + Δ2 JIT tombstone+fat-ptr-sync + Δ4 lower type-prop (O ADMIT sót tầng lower). NARROW: heap-STRUCT trảm (construction-into-field ADR-0067 double-free pre-existing = bom câm). O verify 6 teeth poison-đỏ-restore (count→2/→0 leak/296-297 green/300 coffin-lid). Infra Kỷ-Luật-Gate `9263501` cũng đã O-verify+G-ký phiên này. Nợ mở: heap-struct field-move (chặn ADR-0067), match-arm payload move-out (chặn Lower call-nullable-aggregate). [[campaign_path_separator_and_partial_move]]
