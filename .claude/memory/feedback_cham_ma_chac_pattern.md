---
name: feedback-slow-but-certain-pattern
description: The "Option A" pattern — defer cleanly vs ship temporary. Author repeatedly validated this trade-off in v0.9. Apply when implementation faces a fundamental design-question cliff.
metadata: 
  node_type: memory
  type: feedback
...
**Why:** the author locked it on 2026-05-30: *"better to do 0.10 first and have it complete"*, when the pivot triggered the v0.9 borrow phase split (ADR-0031 §4, Option A). Generalizes to: temporary code = future tech debt, redesign cost > benefit.
...
- **v0.9.x.atomic.7a** Option A — original .7 had .7a/.7b/.7c/.7d/.7e split; added E2420 enforcement minimum to prevent demo teaching anti-patterns. ADR-0031 §4 table split (ship E2420 v0.9, defer NLL v0.10).
...
**Telltale phrase from the author:** "slow but certain — do not ship something temporary that 0.10 will have to fix". When you hear "stability over speed" applied to a scope decision, this pattern is being invoked.
