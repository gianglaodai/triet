...
**Status:** Decided. Applies to v0.3.9+ (serializer/deserializer) and all backends reading `.triv` (CLI, JIT, AOT, trytecode). Official wire format of Triet IR per [ADR-0007 § Wire format](0007-ir-design.md).

**Issue:** ADR-0007 defines the IR shape (register SSA, type-tagged, virtual registers). However, in-memory Rust types (`enum Instruction`, `struct Function`, etc.) are not a wire format — they lack magic bytes, a version field, or a section structure for forward compatibility. A binary format is required that is:

- **Deterministic**: identical `IrProgram` → identical byte sequence (for CAS hash in v0.5).
...
- **Self-describing**: section layout allowing tooling to read without compiler source code.
- **Stable**: frozen at v1.0, backends must be able to read legacy `.triv` files.

This ADR locks the binary format prior to the serialize/deserialize implementation (v0.3.9).
...
2. **32-bit version** (little-endian) — currently `8` (centralized history below).
...
| `7` | v0.10.x.thread.1 ([ADR-0026 v2 §3](0026-actor-boundary-send-rules.md)) | Added 2 raw-thread builtins IDs 43-44 (`RawThreadSpawn`, `RawThreadJoin`). | v6 readers emit `UnknownBuiltin(43/44)`. |
...
3. **Section-based layout** — each section has `section_id` (1 byte) + `section_size` (u32 LE). Unknown section → skip, do not error.
4. **Little-endian** for multi-byte integers (synchronized with primary CPU targets: x86-64, ARM64, RISC-V).
...
| 5–127 | (reserved) | For future Triet versions | — |
...
| SPEC § | Principle | Application to `.triv` |
|--------|-----------|------------------------|
| §0.3.1 | **AI-first** — explicit, low ambiguity | Section layout is self-describing; hex dump readable with section headers. Varint encoding documented — LLMs can generate a parser from this spec. |
| §0.3.4 | **Stability over speed** — ADR-driven | Version field + section skip forward compat; additive-only after v1.0; breaking changes require a new ADR. |
| §0.3.5 | **Refuse over guess** — clear errors | Corrupted file → hard error with diagnostic code; no "best effort" parsing. |
| §0.3.6 | **Explicit > implicit** | Explicit type table dedup; each function declares signature before code; cross-module calls preserve path strings. |
| §0.2 | **Ternary first-class** | Trit/Tryte/Integer/Long encoding is primitive in type table + constant pool; no "flattening to integer". |

### Why LEB128 varint (not fixed-width u32)

- **Compactness**: IR has many small integers (`ValueId`, `BlockId`,
  `ConstId`, counts). Most are < 127 → 1 byte each instead of 4.
- **Prior art**: Wasm, DWARF, Android DEX all use LEB128.
- **Tradeoff**: decoding is ~10-15% slower than fixed-width. Acceptable because
  `.triv` is not on the hot path (not a JIT instruction stream). AOT and JIT
  read `.triv` once at load time, then execute from in-memory IR.

### Why little-endian

- x86-64 and ARM64 (default config) are little-endian. RISC-V is also LE-default.
- Big-endian hardware is no longer common for desktop/server.
- If future ternary hardware (v∞) is big-endian: reader adds `bswap` at
  load time — one-time cost, no runtime impact.

### Why section-based (not flat stream)

- **Forward compatibility**: reader skips unknown sections → older tools can read
  newer files.
- **Parallel parsing**: constant pool and code section can be parsed independently
  (useful for large programs in v0.7+).
- **Tooling**: hex dump, `triet inspect`, debugger can locate sections quickly
  without full parsing.
- **Prior art**: ELF, Wasm, Java class files are all section-based.

### Why separate function table and code section

Separating signature and body:

- **CAS hash (v0.5)**: `iface_hash` only hashes the function table (signatures),
  not code → implementation changes do not invalidate interface hashes.
- **Lazy loading (v0.7+)**: load signatures first to resolve cross-module
  calls, defer code parsing until needed.
- **Tooling**: `triet inspect --signatures` only needs to parse section 3, without
  decoding full instructions.

### Why type table dedup

- Many function params/return types overlap → table dedup avoids re-encoding
  `Integer` 100 times.
- Type index is a permanent ID for a type within the file — used for ABI hashes.

### Why builtin IDs, not full path strings

Builtins (`println`, `assert`, etc.) are a finite, compiler-known set. Encoding
as 1-byte IDs is more compact than the string `"std.io.println"`. When needed
(v0.4 stdlib expansion): add builtin IDs additively. User-defined functions are
called via `call_cross_module` with path strings — keeping the two mechanisms
distinct.
...
Pros: zero implementation effort (`#[derive(Serialize, Deserialize)]`).

Cons:
- **No version field** — serde format changes with struct layout.
  Renaming a field → breaking change. Versioning must be built manually on top.
- **Not self-describing** — tooling cannot read `.triv` without Rust
  struct definitions.
- **No control over encoding** — Compactness and varint decisions are
  dictated by the framework.
- **Not stable** — default `bincode` has no specification; behavior changes between
  versions.

Triet IR must remain stable for 10+ years (through v∞). Coupling wire format to
a specific serde library version is unacceptable.

### A2. Flat buffer (same as in-memory Rust layout)

**Reject.**

- Padding is non-deterministic → cannot hash for CAS.
- Endianness-dependent → non-portable.
- No magic/version/sections → file corruption cannot be detected.

### A3. Text-based (S-expression, JSON, YAML)

**Reject.**

- Too verbose (10-20× binary size) → unsuitable for storage + CAS hashing.
- Parse cost higher than binary varint.
- JSON has no comments → hard to embed debug metadata.
- S-expressions can be used for debug dumps (`triet inspect --text`), but
  are not the canonical format.

### A4. Wasm-style type section + code section

**Consider (partial adoption).**

Wasm uses a type section for function signatures and a code section for bodies —
matching our chosen signature/body separation. But Wasm's type system consists of
value types (i32, i64, f32, f64) — simpler than Triet (Trit, Tryte, Integer,
Long, Trilean, String, Unit, Nullable, struct, enum, closure). Wasm lacks
Ł3/K3 logic ops.

Conclusion: adopt the section layout pattern from Wasm (section ID + size, skip
unknown), but design a dedicated type encoding for Triet.

## Consequences

**Positive:**
- `.triv` is the canonical format for Triet IR — all backends read the same format.
- Version field + section skip provide forward compatibility from day one.
- LEB128 varint is compact — typical function body < 100 bytes.
- Type table deduplication + constant pool deduplication prevent bloat.
- Separating function table / code → makes CAS hashing (v0.5) + lazy loading (v0.7) feasible.
- Magic bytes `triv` enable `file(1)` detection.

**Negative:**
- Binary format is not human-readable → requires `triet inspect` tool for debugging.
- LEB128 decoding cost (although one-time at load time).
- Opcode table must be maintained when adding instructions (additive only, but
  ordering must be preserved).

**Implementation plan:**
- v0.3.9: `triet-ir` module `serde.rs` — `write_program(&IrProgram) -> Vec<u8>`,
  `read_program(&[u8]) -> Result<IrProgram, TrivError>`.
- v0.3.10: CLI `dao build` — parse + lower + serialize → `.triv` file;
  `dao run foo.triv` — deserialize + execute.
- v0.3.11: Round-trip tests: all `examples/*.tri` parse → lower → serialize →
  deserialize → execute → identical output.

**Breaking change policy:**
- Pre-v1.0: opcode renumbering may occur (requires updating reader/writer). After
  v0.3.9: every change must update serializer + deserializer + snapshot
  tests simultaneously.
- Post-v1.0: additive-only — add opcodes, add sections, add type discriminants.
  Do not change semantics of existing opcodes.
...
- ADR-0009 (planned, v0.4): ABI metadata — read from `.triv` function table.
- ADR-0012 (planned, v0.5): CAS hash — `iface_hash` over function table,
  `impl_hash` over code section.
- [ROADMAP § v0.3](../../ROADMAP.md) — Phase deliverables + gates.

---

*This decision freezes the wire format for `.triv`. Serializer/deserializer
implementation is scheduled for v0.3.9. Breaking changes to the binary format require a dedicated ADR.*
