...
Entry state byte: `0 = EMPTY, 1 = OCCUPIED, 2 = TOMBSTONE`.

Lý do chọn byte riêng thay vì sentinel key: Q6 — `i64::MIN` là giá trị hợp lệ
với `V=Integer`. Không thể dùng sentinel key để đánh dấu empty.
...
`K=Integer`: identity hash modulo cap. `hash(k) = k.abs() % cap` (giá trị tuyệt
đối vì Integer có thể âm trong Triết, nhưng i64 có thể là `i64::MIN` — abs của
MIN tràn). Xử lý: `(k % cap + cap) % cap` — ánh xạ Euclidean modulo, luôn
không âm.

Không cần hash phức tạp cho Integer key. Khi String key đến (Bậc C), thay bằng
hash function tổng quát (FNV-1a hoặc SipHash).

### Q3: Collision resolution — open addressing, linear probing

Open addressing với linear probing: `idx = (hash + i) % cap` cho i = 0, 1, 2, …

- Insert: probe đến EMPTY hoặc TOMBSTONE → ghi entry.
- Get: probe đến OCCUPIED với key khớp → trả value; đến EMPTY → không có (trả
  NULL_SENTINEL).
- Remove: KHÔNG có trong scope Bậc B (defer).
...
**Quyết định: REJECT-ON-INSERT.** `insert` từ chối giá trị `i64::MIN` — trả về
lỗi (trap) hoặc return error code. Đây là họ D1 (ADR-0041 §6.2): món nợ
...
- Reject-on-insert là safe: từ chối compile-time cho literal MIN, runtime
  trap cho giá trị động. Không âm thầm.
...
|---------|-----------|------|---------|
| `hashmap_new()` | `-> HashMap` | `__triet_hashmap_alloc(0, 4)` | cap khởi tạo = 4 |
...
| C5 | `insert` với v = i64::MIN → SIGABRT (reject) | Fixture |
...
2. **feat(track-b): HashMap typecheck + lowering** — overload `hashmap_new`,
   `insert`, `get`, `len`; type-string `"HashMap<Integer,Integer>"`
