...
ADR này lock format binary cho ABI metadata + relationship với `.triv` (ADR-0008) và `.tripack` (ADR-0014 sẽ ghi, container format).

## Quyết định

ABI metadata là **một section binary độc lập** trong `.tripack` container, encode theo cùng convention với `.triv` (little-endian, LEB128 varint, length-prefixed UTF-8). Linker đọc **chỉ section này** để quyết định refuse/accept link, không cần load IR code section.
...
for each export:
    ...
    body_offset: varint
      // Offset vào IR code section của .tripack — linker không cần đọc,
...
### 8. Relationship với `.triv` và `.tripack`

| Format | Mục đích | ADR |
|---|---|---|
| `.triv` | IR bytecode của một compilation unit | ADR-0008 |
| ABI metadata | Interface surface, version, hashes, deps | **ADR-0011 (this)** |
| `.tripack` | Container: ABI metadata + N `.triv` units + manifest | ADR-0014 (TBD) |

Linker workflow:
1. Mở `.tripack` → đọc ABI metadata section đầu tiên (cheap).
2. Resolve deps (đọc ABI section của các `.tripack` phụ thuộc).
...
- Linker đọc metadata ~1-10 KB vs full `.tripack` ~100KB-1MB. Cheap version check trước khi load code.
