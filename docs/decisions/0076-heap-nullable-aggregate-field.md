...
## ✚ AMEND — Nợ defer ĐÓNG: nullable-heap field move-out (2026-06-29, G ký)

Mũi 5 ở trên khắc: *"`let s = b.s` (b.s heap) → quăng thẳng E2423... đòi dynamic
drop-flag"*. **Tiền đề "dynamic drop-flag" RÚT LẠI** — chứng minh + cơ chế đóng nằm
ở [ADR-0070 §AMEND Phase 4](0070-partial-move-field-level-move-state.md): slot TỰ LÀ
cờ (STATIC tombstone), drop-side `mir_lower.rs:472` đã anticipate `0 (moved-out)`,
0 `brif`. `let s = b.s` với `b.s : String?`/`Vector?`/`HashMap?` nay **RUN** (fixture
310 FLIP `*_e2423` → `*_run`). Đây là **E2423 source-reachable CUỐI** trên Field-Move-out.
Defer giữ: Index/Deref/Payload non-Field projection (Collection-Semantics).
