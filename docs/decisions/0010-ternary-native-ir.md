...
Bỏ `Constant::Null`. Discriminator của `T?` là một Trit:
- `+1` = Some(value)
- `0`  = null (chính canonical encoding)
- `-1` = reserved (definitely-missing distinct from null — for future "explicit absent" semantics)

`Expr::NullLiteral` lowering: `Const(Trit::Zero)` packed thành nullable.

VM `NullCheck`:
- `RuntimeValue::Null` → `Trit::Zero` (matches discriminator)
- Wrapped value với Trit::Positive → `Trit::Positive` (matches)
- Wrapped value với Trit::Negative → `Trit::Negative` (matches)

Branch then dùng `BrTrilean` trên kết quả NullCheck thay vì BrIf.
