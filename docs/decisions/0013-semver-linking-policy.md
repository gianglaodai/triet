...
Mỗi `.tripack` mang `pkg_version: (u32, u32, u32)` theo ABI metadata (ADR-0011 §1). Triple bumping rules:
...
Tool `triet pack diff old.tripack new.tripack` (v0.4.4+) hiển thị ABI surface diff để hỗ trợ migration. Đây không phải auto-shim — chỉ là human-readable diff tool.
...
- `triet pack inspect foo.tripack` — show metadata (read-only).
- `triet pack diff old.tripack new.tripack` — show ABI diff (v0.4.5+).
