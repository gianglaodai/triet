...
| Compiled artifact | `.tripack` | User-facing — `dao build` output |
...
2. Half-renamed state hiện tại không nhất quán: manifest đã có prefix `triet.`, wire format đã có `.tripack`, nhưng path keyword vẫn là `crate` — gây cognitive split.
...
| 4 | Compiled package artifact | `.tripack` | **`.khi`** | Per §28 phác tán tắc vi khí |
...
| **B** | Wire format `.tripack` → `.khi` (pack serde + store paths + CLI args + docs) | ~50 |
...
- Không support legacy `.tripack` reader — `read_tripack` xóa hoặc rename `read_khi`.
