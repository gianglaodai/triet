...
| Manifest filename | `dao.package` | User-facing — root của project |
| Lockfile | `dao.lock` | User-facing — committed vào VCS |
...
| 6 | Manifest filename | `dao.package` | **`dao.package`** | Cohesive với CLI tool name |
| 7 | Lockfile | `dao.lock` | **`dao.lock`** | Cohesive với CLI tool name |
...
| **D** | Manifest `dao.package` → `dao.package` + lock `dao.lock` → `dao.lock` (loader filename matcher + tất cả demo manifests) | ~30 |
...
- Không support legacy `dao.package` manifest — loader chỉ tìm `dao.package`.
