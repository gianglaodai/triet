...
> **2026-05-31 Addendum (v0.11.0.2) — cache identity LOCKED: canonical `impl_hash_mod`, per-module objects. Author sign-off. Rejects the content-hash shortcut.**
>
> Implementing jit.3 Steps 1–3 surfaced a design decision §4/§7 had glossed: **what keys a cache entry, and at what granularity.** §7's pseudocode wrote `ir.module_impl_hash`, but the IR (`triet_ir::IrModule { path, functions }`) carries **no hash** — there is no module identity at the IR layer. Step 1's `emit_object` therefore emitted one object **per program** (all modules' functions in a single `.o`), with no principled key. This Addendum resolves it before the cache is wired into the dispatcher (Step 4).
>
...
>
> **Supersedes:** Step 1's per-**program** `emit_object` is refactored to per-**module** emission (jit.4a); §4/§7 are now read with this Addendum (key = `impl_hash_mod`, granularity = module). All other sections unchanged.
>
