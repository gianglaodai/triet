...
| [0030](0030-jit-cranelift-integration.md) | JIT integration (Cranelift backend) — refines ROADMAP §v0.9 deliverables. Locks: 3-tier model (Interpreter→VM→JIT, VM persists for cold/warmup/debug), call-count ≥ 100 trigger threshold (Hotspot JVM convention), Cranelift backend với register-SSA 1:1 mapping, BrTrilean → 2 cmp + 2 brnz per ADR-0010 backend, AOT cache `~/.triet/store/jit/{target_triple}/{impl_hash}/`, no capability gate (JIT = same trust as VM), synchronous JIT v0.9 (background defer v1.0+), Stage 2 ≡ Stage 3 byte-identical lift chained to perf gate. Self-host port plan: Layer C runtime, no same-phase port required | Locked |
