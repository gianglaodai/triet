...
| [0031](0031-borrow-expression-syntax.md) | Borrow expression syntax — closes SPEC §10 v0.7 warning ("runtime chưa expose references") + unblocks ADR-0028 §6 example. Locks: prefix `&FORM operand` syntax (all 5 forms `&+`/`&+ mutable`/`&0`/`&0 mutable`/`&-`), operand grammar IDENT/field/index (compound expressions defer), prefix unary precedence tier (right-binding, higher than binary ops, lower than postfix `.`/`[]`/`()`), Type::Reference(form, T) typecheck emission, lowerer passthrough (refs erase at runtime per ADR-0026 v2 §7). Borrow checker enforcement (consume-once, NLL) defers v0.9.x.borrow.*. Self-host port plan: Layer A lockstep mandatory per ADR-0029 §3 — `compiler/parser/parser.tri` mirrors Rust impl same-phase. Implementation sub-phase plan §9 (.7a ADR / .7b Rust impl / .7c self-host port / .7d demo + e2e). | Draft |
