...
Error message cũng theo chuẩn AI-first (ADR-0027): header `EXXXX` + khối fix
mệnh lệnh "Change X to Y" — để cả người và LLM sửa đúng phát đầu.
...
4. **Substrate native cho ML tam phân (BitNet b1.58)** — Microsoft 2024 ("The
   Era of 1-bit LLMs") dùng trọng số ∈ `{-1,0,+1}`; tên *b1.58* vì `log₂3≈1.58`.
   Lượng tử hóa LLM đang đi về đúng balanced ternary. Triết tam phân native +
   AI-first = substrate tự nhiên cho inference ternary-weight (trọng số là `Trit`
   thật, không emulate qua int8). *Trạng thái: Triết chưa có gì về ML — đây là
   câu chuyện định vị, không phải feature. Nối AI-first ↔ ternary thành một mạch.*
...
không viết lại code. Cộng hưởng AI-first (code rõ) + tốc độ DOD, không đánh đổi.
...
- **Nối mầm #4 (BitNet b1.58):** trọng số ML tam phân *là mảng trit*; SoA của
  ternary weights = layout inference tối ưu → `SoA + Trit native + AI-first =
  substrate ML tam phân`. Odin không có mảnh "Trit native"; Triết có.
...
| **D — Native multi-field layout xong** | sau Bậc C/D (hết "mọi value là i64") | **SoA/array programming (Odin)** → rồi **#4 BitNet** | SoA vô nghĩa khi value còn i64; ADR ABI-visibility + aliasing-S6 phải có *trước* |

**Thứ tự cửa:** A song song bất kỳ lúc nào → B / C (không nhất thiết nối tiếp,
tùy thứ tự rebuild Trait vs Capability) → D xa nhất.

**⚠ Kỷ luật ưu tiên (Mentor O):** đường chính hiện tại là **Phase-1 lowering
String → fat-pointer StackSlot** (xem handoff). Mọi mầm trên là *"khi tới cửa thì
xem"*, **không phải làm ngay**. Cám dỗ lớn nhất là kéo SoA/BitNet lên sớm vì
chúng "kêu" — nhưng chúng ở Cửa D, xa nhất. Cửa duy nhất động được ngay mà không
tạo nợ là **Cửa A**, và chỉ ở mức *viết ADR chốt tính chất*, không implement.
