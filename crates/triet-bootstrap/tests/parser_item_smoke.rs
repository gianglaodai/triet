//! v0.7.5.4a — smoke test for the Triết-in-Triết item layer
//! (top-level driver + functions + constant + type alias +
//! visibility) at `compiler/parser/parser.tri`.
//!
//! Builds the source to `.triv`, round-trips through the wire
//! reader, then runs `main()` end-to-end on the VM. Beyond the
//! v0.7.5.{1,2,3} coverage, the Triết-side `main()` now exercises
//! the full item-level surface: `function foo() = expr` /
//! `function foo() { block }`, generic params (`<T, U>`),
//! parameter passing modes (default / `mutable` / `owned`),
//! optional return type (`-> T`), `constant NAME (: T)? = expr`,
//! `type Alias = T`, and the visibility prefixes (`public`,
//! `public(package)`).
//!
//! The struct / enum / module / import items are stubbed at
//! `parse_item` dispatch and surface as parser errors here — they
//! ship in v0.7.5.4b.
//!
...
#[test]
fn parser_item_smoke_main_passes_all_asserts() {
    ...
    // parser.tri's smoke main, whose v0.7.5.4a surface now runs all
    // item-layer asserts.
