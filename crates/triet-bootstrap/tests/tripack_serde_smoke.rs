//! v0.7.9.3 — `.tripack` round-trip smoke test.
//!
//! Loads `compiler/pack_writer.tri` (which now hosts both the .triv
//! writer from v0.7.9.2 and the .tripack wrapper from v0.7.9.3),
//! invokes `tripack_smoke_main()` inside the VM, and then asserts
//! that the Triết-emitted .tripack bytes round-trip cleanly through
//! Rust `triet_pack::read_tripack`.
...
#[test]
fn tripack_smoke_main_returns_positive_true() {
    let ir = pack_writer_ir().clone();
    let func_id = lookup_func(&ir, "tripack_smoke_main");
    let mut vm = Vm::new(ir);
    let result = vm
        .execute(func_id, vec![])
        .expect("tripack_smoke_main must execute without VM error");
...
/// Invoke `serialize_source_to_tripack` for a simple Triết source and
/// confirm the result decodes via Rust `triet_pack::read_tripack`. The
/// Rust decoder validates the magic bytes + ABI version + section
/// framing, so a clean round-trip proves the wire format matches.
#[test]
fn rust_read_tripack_decodes_triet_emitted_bytes() {
    let ir = pack_writer_ir().clone();
    let func_id = lookup_func(&ir, "serialize_source_to_tripack");
    let mut vm = Vm::new(ir);
    ...
    let result = vm
        ...
        .expect("serialize_source_to_tripack execution");
    ...
    assert_eq!(
        &bytes[..4],
        b"trip",
        "Triết tripack output must start with `trip` magic"
    );

    // Rust decodes the Triết bytes.
    let (metadata, code_section) = read_tripack(&bytes)
        .expect("Rust read_tripack must accept Triết-emitted .tripack");
